...
class Tof:
    def __init__(
        ...
        usb_manager,
        buffer_size: int = 5,
        background_shape=(100, 100),
        matrix_path=None,
        baudrate=921600,
        rotation=0,
    ):
        self.addr = usb_manager.get_tty_by_devicename(name)
        self.name = name
    ...
    def _read_data(self):
        ...
        while True:
            ...
            try:
                ...
                print("Tof >> Error reading serial: ", e)
                to_add = bytes()
            if not to_add:
                time.sleep(0.02)
                if last < time.time() - 1:
    ...
    def get_image(self, pop: bool = True, rotation: bool = False):
        # print("Tof >> Getting image, buffer size: ", len(self.images))
        if len(self.images):
            image = (list.pop if pop else list.__getitem__)(self.images, 0)
            if rotation:
                # rotate with numpy for the number of times given by self.rotation, it could be negative
                image = numpy.rot90(image, self.rotation)
        else:
            return None
