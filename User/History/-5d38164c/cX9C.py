...
class Tof:
    ...
    def _read_data(self):
        while True:
            print(f"{self.name} >> Starting {self.addr[self.current_addr]}")
            time.sleep(0.5)
            self.reload_addresses()
            self.reload_serial_connection(close=False)
            try:
                last = time.time()
                start = time.time()
                c = 0
                    ...
                    if not to_add:
                        ...
                        if last < time.time() - 1:
                            ...
                            if retries > 5:
                                ...
                                print(f"{self.name} >> No data for 1 second, turning to {self.addr[self.current_addr]}")
                            last = time.time()
                        continue
                    last = time.time()
                    self.data += to_add
    ...
    def reload_serial_connection(self, close=True):
        if close:
