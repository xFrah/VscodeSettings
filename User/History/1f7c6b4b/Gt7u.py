...
class trading_env:
    ...
    def _short(self, open_posi, enter_price, current_mkt_position, current_price_mean):
        if open_posi:
            ...
            after_act_mkt_position = current_mkt_position - 1
            self.chg_price_mean[:] = (current_price_mean * abs(current_mkt_position) + enter_price) / abs(after_act_mkt_position)
            self.chg_posi[:] = after_act_mkt_position
            self.chg_posi_var[:1] = -1
            self.chg_posi_entry_cover[:1] = 2
    ...
    def step(self, action):
        ...
        if self.step_st + self.obs_len + self.step_len >= len(self.price):
            ...
            if current_mkt_position != 0:
                ...
                self.chg_makereal[:1] = 1
                self.chg_reward[:] = (
                    (self.chg_price - self.chg_price_mean) * (current_mkt_position) - abs(current_mkt_position) * self.fee
                ) * self.chg_makereal
            self.transaction_details = pd.DataFrame(
                [
                    self.posi_arr,
                ...
                index=["position", "position_variation", "entry_cover", "price_mean", "reward_fluctuant", "reward_makereal", "reward"],
    ...
    def _gen_trade_color(
        self, ind, long_entry=(1, 0, 0, 0.5), long_cover=(1, 1, 1, 0.5), short_entry=(0, 1, 0, 0.5), short_cover=(1, 1, 1, 0.5)
    ):
    ...
    def _plot_trading(self):
        ...
        self.features_plot = [
            self.ax3.plot(price_x, self.obs_features[: self.step_st + self.obs_len, i], c=self.features_color[i])[0]
            for i in range(self.feature_len)
        ]
        rect_high = self.obs_price.max() - self.obs_price.min()
        self.target_box = self.ax.add_patch(
            patches.Rectangle(
                (self.step_st, self.obs_price.min()),
                self.obs_len,
                rect_high,
                label="observation",
                edgecolor=(0.9, 1, 0.2, 0.8),
                facecolor=(0.95, 1, 0.1, 0.3),
                linestyle="-",
                linewidth=1.5,
                fill=True,
            )
        )  # remove background)
        self.fluc_reward_plot_p = self.ax2.fill_between(
            price_x,
            0,
            self.reward_fluctuant_arr[: self.step_st + self.obs_len],
            where=self.reward_fluctuant_arr[: self.step_st + self.obs_len] >= 0,
            facecolor=(1, 0.8, 0, 0.2),
            edgecolor=(1, 0.8, 0, 0.9),
            linewidth=0.8,
        )
        self.fluc_reward_plot_n = self.ax2.fill_between(
            price_x,
            0,
            self.reward_fluctuant_arr[: self.step_st + self.obs_len],
            where=self.reward_fluctuant_arr[: self.step_st + self.obs_len] <= 0,
            facecolor=(0, 1, 0.8, 0.2),
            edgecolor=(0, 1, 0.8, 0.9),
            linewidth=0.8,
        )
        self.posi_plot_long = self.ax2.fill_between(
            price_x,
            0,
            self.posi_arr[: self.step_st + self.obs_len],
            where=self.posi_arr[: self.step_st + self.obs_len] >= 0,
            facecolor=(1, 0.5, 0, 0.2),
            edgecolor=(1, 0.5, 0, 0.9),
            linewidth=1,
        )
        self.posi_plot_short = self.ax2.fill_between(
            price_x,
            0,
            self.posi_arr[: self.step_st + self.obs_len],
            where=self.posi_arr[: self.step_st + self.obs_len] <= 0,
            facecolor=(0, 0.5, 1, 0.2),
            edgecolor=(0, 0.5, 1, 0.9),
            linewidth=1,
        )
        self.reward_plot_p = self.ax2.fill_between(
            price_x,
            0,
        ...
        self.reward_plot_n = self.ax2.fill_between(
            price_x,
            0,
            self.reward_arr[: self.step_st + self.obs_len].cumsum(),
            where=self.reward_arr[: self.step_st + self.obs_len].cumsum() <= 0,
        ...
        self.trade_plot_buy = self.ax.scatter(
            x=trade_x_buy, y=trade_y_buy, s=100, marker="^", c=trade_color_buy, edgecolors=(1, 0, 0, 0.9), zorder=2
        )
        self.trade_plot_sell = self.ax.scatter(
            x=trade_x_sell, y=trade_y_sell, s=100, marker="v", c=trade_color_sell, edgecolors=(0, 1, 0, 0.9), zorder=2
        )
