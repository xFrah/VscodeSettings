...
static int mqtt_message_buffer_max = sizeof(struct tag) * 64;
static char * mqtt_message_buffer[mqtt_message_buffer_max] = {0};
...
static void event_handler(void *arg, esp_event_base_t event_base,
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        while (hashmap_iter(tag_hashmap, &iter, &item)) {
            ESP_LOGI(TAG, "Iterating through hashmap, iter=%zu", iter);
            ...
            if (tag_ptr != NULL) { // Check if the tag pointer is valid
                ESP_LOGI(TAG, "Tag %d: %s", i, tag_ptr->id);
        ...
        // put { as the first character of the message, we construct a JSON array
        mqtt_message_buffer[0] = '{';
        // store current pointer that will be offset by the length of the message
        char *current_pointer = mqtt_message_buffer + 1;
        int remaining;
        int written;

        for (size_t i = 0; i < tag_count; i++) {
            remaining = mqtt_message_buffer_max - (current_pointer - mqtt_message_buffer);
        ...
        *(current_pointer - 1) = '}';
        int total_length = current_pointer - mqtt_message_buffer;
        ESP_LOGI(TAG, "End of hashmap iteration");
