...
class PriceDataCalculator:
    ...
    def load_prices(self) -> pd.DataFrame:
        ...
        self.prices = pd.read_csv(
            f'data/prices/input_pricedata/{self.symbol}_M15.csv', index_col='Date', parse_dates=True)

    def compute_bollinger_bands(self):
        self.prices['Simple Moving Average (20)'] = self.prices['Close'].rolling(
            window=20).mean()
        self.prices['Standard Deviation (20)'] = self.prices['Close'].rolling(
            window=20).std()
    ...
    def compute_macd(self):
        self.prices['Exponential Moving Average (12)'] = self.prices['Close'].ewm(
            span=12).mean()
        self.prices['Exponential Moving Average (26)'] = self.prices['Close'].ewm(
            span=26).mean()
        self.prices['MACD Line'] = self.prices['Exponential Moving Average (12)'] - \
            self.prices['Exponential Moving Average (26)']
        self.prices['MACD Signal'] = self.prices['MACD Line'].ewm(
            span=9).mean()
    ...
    def compute_rsi(self):
        ...
        self.prices['Previous Day Gain'] = self.prices['Previous Day Change'].mask(
            self.prices['Previous Day Change'] < 0, 0.0)
        # loss is the difference if negative, else 0
        self.prices['Previous Day Loss'] = -self.prices['Previous Day Change'].mask(
            self.prices['Previous Day Change'] > 0, -0.0)
        # average gain is the average of the gains for the last 14 days
        self.prices['Average Gain EWM (13,14)'] = self.prices['Previous Day Gain'].ewm(
            com=13, min_periods=14).mean()
        # average loss is the average of the losses for the last 14 days
        self.prices['Average Loss EWM (13,14)'] = self.prices['Previous Day Loss'].ewm(
            com=13, min_periods=14).mean()
        self.prices['Relative Strength'] = self.prices['Average Gain EWM (13,14)'] / \
            self.prices['Average Loss EWM (13,14)']
        self.prices['Relative Strength Index'] = 100 - \
            (100 / (1 + self.prices['Relative Strength']))

    def compute_awesome_oscillator(self):
        self.prices['Simple Moving Average (5)'] = self.prices['Close'].rolling(
            window=5).mean()
        self.prices['Simple Moving Average (34)'] = self.prices['Close'].rolling(
            window=34).mean()
        self.prices['Awesome Oscillator'] = self.prices[
            'Simple Moving Average (5)'] - self.prices['Simple Moving Average (34)']

    def compute_stochastic_oscillator(self):
        # this code computes the stochastic oscillator value for each day
        self.prices['Lowest Low (14)'] = self.prices['Low'].rolling(
            window=14).min()
        self.prices['Highest High (14)'] = self.prices['High'].rolling(
            window=14).max()
        self.prices['Stochastic Fast Signal (%K)'] = 100 * ((self.prices['Close'] - self.prices['Lowest Low (14)']) / (
            self.prices['Highest High (14)'] - self.prices['Lowest Low (14)']))
        self.prices['Stochastic Slow Signal (%D)'] = self.prices['Stochastic Fast Signal (%K)'].rolling(
            window=3).mean()
    ...
    def compute_accumulation_distribution(self):
        self.prices['Accumulation Distribution'] = ((self.prices['Close'] - self.prices['Low']) - (
            self.prices['High'] - self.prices['Close'])) / (self.prices['High'] - self.prices['Low'])

    def compute_parabolic_sar(self):
        # this code computes the parabolic sar value for each day
        indicator = PSAR()
        self.prices['PSAR'] = self.prices.apply(
            lambda x: indicator.calcPSAR(x['High'], x['Low']), axis=1)
        # Add supporting data
...
def make_dataset(symbol):
    ...
    final_dataset = make_dataset_prices_news(f'data/prices/working_pricedata/{symbol}_M15_prices_dataset.csv','data/news/forexlive.csv', symbol)
