...
def make_dataset(symbol: str, interval: int) -> None:
    ...
    # Group points by day
    for point in points:
        time2vec_res = time2vec_embedding(point[0]) # convert timestamp to time2vec embedding
        point.append(time2vec_res) # append time2vec embedding to point
    ...
    # remove first element of each point
    points_by_day = [[point[1:] for point in day] for day in points_by_day]
