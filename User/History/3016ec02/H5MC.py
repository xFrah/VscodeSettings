from helpers import reset_with_timestamp, get_mac_address, get_ble_mac, get_configuration
...
def main():
    ...
    try:
        ...
        collect()
        set_time()
        check_for_updates(config["badge_update_url"], config["update_hours"], mac)
        mqtt = MQTTClient(mac, config["mqtt_ip"], port=config["mqtt_port"])
        mqtt.connect()
    ...
    while True:
        ...
        if sec_obj.authed == still_the_same and mqtt:
