...
class UHF:
    ...
    def get_tx_power(self, wait=0.1) -> int:
        self._write(b"\xBB\x00\xB7\x00\x00\xB7\x7E", wait=wait)

    def set_tx_power(self, power: int, wait=0.1) -> None:
        """
    ...
    def get_region(self, wait=0.1) -> int:
        """
        ...
        while time.time() - start < timeout:
            ...
            if other:
                for packet in other:
                    print(packet)
        self.stop_scan()
        # compute average rssi for each tag, using last 10 values
    ...
    def scan(self, wait=0.1) -> list[Tag]:
        """
        Scan for rfid tags nearby and returns a list of tags
        """
    ...
    def _decode(self) -> None:
        ...
        if packet_type == TYPE_NOTICE and command == COMMAND_NOTICE:
            ...
            if command == COMMAND_GET_TX_POWER:
                ...
                self._data["msg"] = "Region is " + REGION_CODES[self._data["region"]] if self._data["region"] in REGION_CODES else self._data["region"] + "."
