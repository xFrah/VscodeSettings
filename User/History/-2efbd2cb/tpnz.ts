    ...
    project?.forEach((projectWorker) => {
        ...
        if (projectWorker?.tasks?.length > 0) {
            projectWorker?.tasks?.forEach((taskDetails) => {
                ...
                const overtimeMap = calculateHourlyOvertimePercentage(taskDetails?.start_time, taskDetails?.end_time, WORKER_TYPE === "EMPLOYEE");
                const tasksOvertime = countOvertimeOccurrences(overtimeMap);
                ordinary_hours += GET_TASK_CALCULATION["ordinaryHours"]
                overtime_hours += GET_TASK_CALCULATION["overtimeHours"]
                totalTime += minutes;
            });
