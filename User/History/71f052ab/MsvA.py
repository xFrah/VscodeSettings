...
def get_output_device_names():
    """
    Get the names of the audio output devices.
    """
    try:
        return [device["name"] for device in sd.query_devices() if device["max_output_channels"] > 0 and device["hostapi"] == 3]
...
class Interface:
    def __init__(self, name, guid: str) -> None:
        self.guid = guid
        self.name = name
...
class Interfaces:
    def __init__(self) -> None:
        self.guids: list[str] = []
        try:
            self.guids = get_output_device_names()
            print("[AU] GUIDS:", self.guids)
        except Exception as e:
            print(e)

        t_int = self.load_interface_names()
        self.interfaces: list[Interface] = [Interface(t_int.get(guid, guid), guid) for guid in self.guids]
        self.update_interface_names(t_int)
    ...
    def get_interface_index_by_guid(self, guid: str):
        for i, interface in enumerate(self.interfaces):
            if interface.guid == guid:
                print("[AU] Found interface", interface.name, "at index", i)
                return i
            print("[AU] Interface", interface.name, "at index", i, "does not match", guid)
        return None
    ...
    def reload_interfaces(self):
        ...
        """
        t_int = self.load_interface_names()
        self.interfaces = [Interface(t_int[guid], guid) for guid in self.guids]
        self.update_interface_names(t_int)

    def rename_interface(self, guid, name):
        """
        Rename an interface.
        """
        # get the current translation from the file
...
def get_audio_tracks_info(movie_file):
    ...
    try:
        ...
        print("[AU] command '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output))
        return []
