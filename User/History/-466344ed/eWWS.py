        ...
        while True:
            ...
            if next_session:
                ...
                if current_timestamp >= next_session["end"]:
                    ...
                    cv2.imwrite(os.path.join(cache_path, f"b{time.time():.2f}.png"), current_background)
                    # write frame
                    cv2.imwrite(os.path.join(cache_path, f"b{time.time():.2f}.png"), frame)
...
if __name__ == "__main__":
    # check if l2 folder exists
    if not os.path.exists("L2_RESULT"):
        print("L2_RESULT folder not found.")
        # create folder
        os.makedirs("L2_RESULT")
