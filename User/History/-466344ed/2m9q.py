...
def detect_changes(cache_path, roi_path, mog_history=1000, mog_var_threshold=128, delete_useless_videos=True):
    ...
    for json_file in enumerate(json_files):
        ...
        first_session = sessions[0] if 'sessions' in data and data['sessions'] != [] else None
        ...
        while True:
            ...
            print(f'Frame {frame_count}, {current_timestamp} seconds, {len(sessions)} sessions left.')
            
            if first_session:
                # check if we have reached the next session
                if current_timestamp >= first_session['start']:
                    print('First session start:', next_session['start'], ", getting background image.")
                    backgrounds.append(mog2.getBackgroundImage())
                    next_session = sessions[0] if len(sessions) > 0 else None
