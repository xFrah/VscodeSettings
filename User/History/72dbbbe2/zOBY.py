...
# final_dict = {}
# cecg, ceda, ctemp, cresp, label, weda, wtemp, wbvp = [], [], [], [], [], [], [], []
# for i in range(2, 12):
#     print(i)
#     file_ = pd.read_pickle(data + f"\\S{i}\\S{i}.pkl")
#     cecg.extend(file_["signal"]["chest"]["ECG"][:, 0])
#     ceda.extend(file_["signal"]["chest"]["EDA"][:, 0])
#     ctemp.extend(file_["signal"]["chest"]["Temp"][:, 0])
#     cresp.extend(file_["signal"]["chest"]["Resp"][:, 0])
#     label.extend(file_["label"])
#     weda.extend(file_["signal"]["wrist"]["EDA"][:, 0])
#     wtemp.extend(file_["signal"]["wrist"]["TEMP"][:, 0])
#     wbvp.extend(file_["signal"]["wrist"]["BVP"][:, 0])
...
# write everything to csv file. for each prediction, write a line.
# each line is(separated by comma):
# - ecg
# - eda
# - temp
# - resp
# - probabilities for each class(separated by comma)

# save model to file
pickle.dump(model_allChest, open("model_allChest.pkl", "wb"))
...
print(len(x_test))
# do argmax from allchest_pred
allChest_pred = np.argmax(allChest_pred, axis=1)
with open("predictions.csv", "w") as f:
    for i in range(len(allChest_pred)):
        stringa = (
            ...
            + ","
            + str(allChest_pred[i][0])
            + ","
            + str(allChest_pred[i][1])
            + ","
            + str(allChest_pred[i][2])
            + ","
            + str(allChest_pred[i][3])
            + ","
            + str(allChest_pred[i][4])
            + "\n"
        )
        f.write(stringa)
        print(stringa)
