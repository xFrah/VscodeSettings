...
xFrah
/
yolov5-anti-collision
Private
Fork your own copy of xFrah/yolov5-anti-collision
Code
Issues
Pull requests
Actions
...
yolov5-anti-collision/detect.py /
@xFrah
xFrah Everything works.
Latest commit 4f1dee8 2 weeks ago
 History
 1 contributor
...
def run(
    ...
    lidar = LIDAR.Lidar(slices=True, buffer_size=1)
    lidar.active(True)
    source = str(source)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if view_img:
                ...
                start = time.time()
                # get the distances from the lidar
                distances = lidar.get_distances()
                # get the slices from the lidar
                slices = lidar.get_slices()
                ...
                if distances and slices:
                    ...
                    for person_angle in people_angles:
                        distance = lidar.get_distance_at_angle(slices, person_angle)
                        # convert distance and angle to x and y coordinates
