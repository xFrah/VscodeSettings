...
def caseAlterations(dataFile, labelFile, sampleRates):
    ...
    sigData["ECG"] = (
        sigData["ECG"]
        .apply(lambda x: ((x - 2.8) / 50) * sampleRates["ecg"])
        .round(decimals=3)
    )
    ...
    with open(dataFile, "rb") as f:
        ...
        if dataFile.endswith(".pkl"):
            ...
            try:
                ...
                print(
                    f"ERROR - Please ensure the file {dataFile} includes the correct keys!"
                )
...
def processRawData(
    ...
    if dataset.lower().strip() == "case":
        ...
        for subject in dataFiles:
            ...
            # Windowing
            ecgWindows = getLabelledWindows(
                ecg, label, windowSize, sampleRates["ecg"], sampleRates["label"], "ECG"
            )
            ppgWindows = getLabelledWindows(
                ppg, label, windowSize, sampleRates["ppg"], sampleRates["label"], "PPG"
            )
        ...
        if f.endswith(str(dataKey) + ".csv"):
            ...
            try:
                ...
                if resDf is None:
                    ...
                    # in new versions append is deprecated, use concat
...
def cleanDataFrame(df):
    ...
    """
    # clean data
    indexes = df.index[~df.isin([np.nan, np.inf, -np.inf]).any(1)]
