...
function AnalysisCharts({ ProjectData }) {
  ...
  // generate 10 ticks for the hAxis, starting from 0 to maxAxisValue
  const ticks = Array.from(Array(10).keys()).map((i) => {
    const tick = (i * maxAxisValue) / 10;
    return tick;
  });
        ...
        <View className="flex flex-row w-full">
          <View className="flex flex-row w-2/3">
            <Chart
              ...
              data={data2}
              options={{
                title: `Total Budget of Project ${ProjectData?.getCostMarginBudgetDashboardDetails?.budget} ${APP_CURRENCY}, Margin ${fixedCost} ${APP_CURRENCY}, Over Budget ${overBudgetCost} ${APP_CURRENCY}`,
                chartArea: { width: "70%" },
                isStacked: true,
