    ...
    def read_nfc(self, timeout=500):
        ...
        while True:
            ...
            if uid is not None:
                ...
                res = auth(string_ID)
                if res and time.time() - self.sec_obj.timestamp_opened > 10:
                    if self.sec_obj.closed:
                        self.sec_obj.closed = False
                        self.sec_obj.timestamp_opened = time.time()
                        print("[NFC] Opening door.")
                    else:
                        self.sec_obj.closed = True
                        self.sec_obj.timestamp_opened = time.time()
                        print("[NFC] Closing door.")
            else:
                if not self.sec_obj.closed and time.time() - self.sec_obj.timestamp_opened > 60:
                    self.sec_obj.closed = True
