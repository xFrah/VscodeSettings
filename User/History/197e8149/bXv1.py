...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_sentences: Tensor, src_characteristics_vector: Tensor, src_mask: Tensor = None) -> Tensor:
        ...
        if torch.isnan(output).any():
            print("NaN values in first transformer output")
        ...
        # Check if outputs contain NaN values
        if torch.isnan(output).any():
            print("NaN values in conv1 output")

        if torch.isnan(src_characteristics_vector).any():
            print("NaN values in src_characteristics_vector")
        ...
        # Check if outputs contain NaN values
        if torch.isnan(combined_embeddings).any():
            print("NaN values in combined embeddings")
        ...
        output_3 = output_2.permute(0, 2, 1)  # output shape [1, d_model, seq_len]
        ...
        price_prediction = nn.AdaptiveAvgPool1d(1)(output_3)  # output shape [1, final_conv_channels[-1], 1]
...
if __name__ == "__main__":
    ...
    def get_slice_at_index(index, batch_size) -> tuple[Tensor, float, Tensor]:
        ...
        label = (df.iloc[label_index]["Close"] - df.iloc[label_index - 1]["Close"]) * 10000
    ...
    for epoch in range(epochs):
        for batch_idx in range(num_batches):
            ...
            if torch.isnan(df_slice).any():
                print("NaN values in df_slice")
                continue
