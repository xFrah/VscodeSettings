...
def __init__(
    ...
    d_hid: int,
    nlayers: int,
    num_classes: int = 5,  # Number of classes for multi-class classification
    dropout: float = 0.5,
):
    super().__init__()
    ...
    # Adding Attentive Pooling Layer
    self.att_pool = AttentivePooling(d_model)
    self.linear = nn.Linear(d_model, num_classes)  # Output dimension is now num_classes
...
def forward(self, src_characteristics_vector: Tensor, src_mask: Tensor = None) -> Tensor:
    ...
    # Final output: [seq_len, num_classes]
    final_output = self.linear(pooled_output)

    return softmax(final_output, dim=1)  # Apply softmax to obtain probability distribution
