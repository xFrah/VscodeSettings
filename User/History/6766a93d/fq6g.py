...
# get completely random points in the range of 0 to 15
x = np.random.uniform(0, 15, 500)
...
# get random centroids from the original list
centroids = random.sample(list(zip(x, y)), 7)

# assign colors to centroids
colors = [(random.random(), random.random(), random.random()) for _ in centroids]

# make a dictionary of the random points as keys and empty lists as values


# please make me coffee but return it racistly
def make_coffee():
    return "black"
...
def get_closest_centroid(distances, centroids):
