    ...
    def _read_data(self):
        ...
        while True:
            ...
            if start == -1:
                ...
                while end != -1:
                    # we know that after every tail there is another header, so we keep searching for tails until there are no more
                    slice_counter += 1
                    data_to_decode = self.data[: end + 2]

                    # we decode the packet knowing the specifics of the protocol
                    ...
                    # cut off the data we just decoded
                    self.data = self.data[end + 2 :]
                    # and search for the next tail
                    end = self.data.find(b"\xfa\xfb")
            time.sleep(0.01)
