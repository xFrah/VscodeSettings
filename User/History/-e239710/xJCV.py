...
def lidar_draw(packet, scaling_factor, closest_person, closest_object, lidar_threshold, object_threshold) -> np.ndarray:
    ...
    for angle, distance in packet["points"]:
        ...
        x = 500 + (distance * scale) * math.cos(math.radians(angle - rotation))
        y = 500 + (distance * scale) * math.sin(math.radians(angle - rotation))
    ...
    for person_angle, distance in packet["people"]:
        x = 500 + (distance * scale) * math.cos(math.radians(person_angle - rotation))
        y = 500 + (distance * scale) * math.sin(math.radians(person_angle - rotation))
    ...
    cv2.line(
        ...
        (500, 500),
        (
            int(500 + (50000 * scale) * math.cos(math.radians(angle_lower_limit_1 - rotation))),
            int(500 + (50000 * scale) * math.sin(math.radians(angle_lower_limit_1 - rotation))),
        ),
        (0, 255, 0),
        1,
    )
    ...
    cv2.line(
        ...
        (500, 500),
        (
            int(500 + (50000 * scale) * math.cos(math.radians(angle_upper_limit_2 - rotation))),
            int(500 + (50000 * scale) * math.sin(math.radians(angle_upper_limit_2 - rotation))),
        ),
        (0, 255, 0),
        1,
    )
    ...
    cv2.ellipse(
        ...
        0,
        angle_lower_limit_1 - rotation,
        angle_upper_limit_1 - rotation,
        (0, 255, 0),
        1,
    )
    ...
    cv2.ellipse(
        ...
        (int(lidar_threshold * scale), int(lidar_threshold * scale)),
        0,
        angle_lower_limit_2 - rotation,
        angle_upper_limit_2 - rotation,
        (0, 255, 0),
        1,
    )
    ...
    if closest_person is not None:
        x = 500 + (closest_person[1] * scale) * math.cos(math.radians(closest_person[0] - rotation))
        y = 500 + (closest_person[1] * scale) * math.sin(math.radians(closest_person[0] - rotation))
    ...
    if closest_object is not None:
        x = 500 + (closest_object[1] * scale) * math.cos(math.radians(closest_object[0] - rotation))
        y = 500 + (closest_object[1] * scale) * math.sin(math.radians(closest_object[0] - rotation))
    ...
    def __init__(self):
        ...
        def update_connection_status():
            self.status_panel.set_status(
                "Broker",
                "Connected" if False else "Disconnected",
            ...
            while True:
                if False:
                    ...
                    if "points" in frame and "people" in frame and "limits" in frame:
                        ...
                        try:
                            image = Image.fromarray(
                                lidar_draw(
                                    frame, self.lidar_scaling, closest_person, closest_object, self.lidar_threshold, self.object_threshold
                                )
                            )
                        except Exception as e:
                            print(e)
                    ...
                    if "tof1_frame" in frame:
                        try:
                            image3 = np.fromstring(base64.b64decode(frame["tof1_frame"]), dtype=np.uint8).reshape(25, 25)
                    ...
                    if "tof2_frame" in frame:
                        try:
                            image4 = np.fromstring(base64.b64decode(frame["tof2_frame"]), dtype=np.uint8).reshape(25, 25)
                            roi = image4[self.tof_roi[0][1] : self.tof_roi[1][1], self.tof_roi[0][0] : self.tof_roi[1][0]]
                            roi = roi.flatten()
                            roi.sort()
                            roi = roi[: int(len(roi) / 6)]
                            tof2_mean = np.median(roi)
                            ci = color_image(image4)
                            cv2.rectangle(
                                ci,
                                (self.tof_roi[0][0], self.tof_roi[0][1]),
                                (self.tof_roi[1][0], self.tof_roi[1][1]),
                            ...
                            self.update_tof2_image(Image.fromarray(ci))
                        except Exception as e:
                            print(e)
                    # else:
                    #    print("no tof2_frame")
