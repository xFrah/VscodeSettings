...
def requires_form_entries(args: Dict[str, type]) -> List[str]:
    """Check if all the arguments are present in the request."""
    args_list = []
    for arg, t in args.items():
        ...
        if not isinstance(request_arg, t):
            raise ValueError(f"Invalid type for {arg}.")
        args_list.append(request_arg)
    return args_list
...
def get_kit_info():
    ...
    [kit_mac_address] = requires_form_entries({"kit_mac_address": str})
...
def register_item():
    ...
    [kit_mac_address, item_type, expiration_date] = requires_form_entries(
        {"kit_mac_address": str, "item_type": str, "expiration_date": str}
    )
