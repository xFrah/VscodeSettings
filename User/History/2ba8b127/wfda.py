...
def get_kit_info():
    ...
    missing = {e[0] for e in db.get_disappearing_items(kit_mac_address).items() if (datetime.datetime.now() - e[1]).total_seconds() > 15}

    for p in products:
        if p.product_id not in missing:
            occurrences[p.product_id] -= 1
    ...
    kit_info_dict: Dict[str, Any] = {
        ...
        "is_offline": kit_mac_address in db.kit_reports,
    }
...
def register_item():
    ...
    if item_type not in kit_columns:
        print(f"Invalid item_type: {item_type}")
        abort(400, description="Invalid item_type.")
    ...
    if not possible_item_ids:
        print("No unregistered items found for kit.")
        abort(412, description="No unregistered items found for kit.")  # 412 Failed Dependency
...
def register_kit_kitside():
    ...
    """
    # requires api token in form
    [api_token] = requires_form_entries("api_token")
