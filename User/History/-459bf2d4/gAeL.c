...
void decode(uint8_t *raw) {
    ...
    if (packet_type == TYPE_NOTICE && command == COMMAND_NOTICE) {
        ...
        // put formatted EPC into a string
        char epc[25];
        sprintf(epc, "%02x%02x%02x%02x%02x%02x%02x%02x", raw[8], raw[9], raw[10], raw[11], raw[12], raw[13], raw[14], raw[15]);
        struct tag newTag;
        memcpy(newTag.id, &raw[8], 12);
        newTag.id[12] = '\0';
        newTag.timestamp = esp_timer_get_time();
        // print tag id and timestamp
        printf("ID: %s, Timestamp: %d", newTag.id, newTag.timestamp);
        if (hashmap_set(tag_map, &newTag) == NULL) {
            ESP_LOGE(TAG, "Failed to add tag to map!");
        }

        if (hashmap_oom(tag_map)) {
            ESP_LOGE(TAG, "Tag map is out of memory!");
        }
        ...
        if (xQueueReceive(uart0_queue, (void *)&event, (TickType_t)portMAX_DELAY)) {
            ...
            case UART_DATA:
                ...
                while ((header_index = search_for_bytes(data_buffer, data_buffer_size, (uint8_t *)"\xBB", 1, processed)) >= 0) {
                    vTaskDelay(1);
                    if (processed >= data_buffer_size) {  // we clear the buffer
                        ESP_LOGI(TAG, "Processed all bytes");
                        break;
                    }
                    if (header_index < 0) {  // we kill the rest of the buffer
                        ESP_LOGI(TAG, "No header found");
                        break;
                    }
                    if (data_buffer_size - header_index < 5) { // We keep those bytes and wait for more
...
struct hashmap *rfid_init(void) {
    ...
    // add dummy tag to map
    struct tag dummy_tag;
    strcpy(dummy_tag.id, "000000000000");
    dummy_tag.timestamp = 0;
    if (hashmap_set(tag_map, &dummy_tag) == NULL) {
        ESP_LOGE(TAG, "Failed to add dummy tag to map!");
    }
