...
export default {
  ...
  async mounted() {
    ...
    this.tempIdwLayer.getSource().addFeatures(this.waypointFeatures);

    const TempImageLayer = new Image({
      title: 'IDW',
      source: this.tempIdwLayer,
      opacity: .5,
      visible: false // Initially hidden
    });
    ...
    websocketData(newData) {
      if (newData) {
        ...
        if (newData.markers) {
          ...
          this.tempIdwLayer.getSource().clear();
          this.tempIdwLayer.getSource().addFeatures(waypointsUntilCurrent.map(wp => {
            const feature = new Feature({
              geometry: wp.geometry,
              val: wp.temperature,
            });
