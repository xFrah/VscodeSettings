      ...
      if (newData) {
        ...
        if (newData.markers) {
          ...
          const waypointFeatures = waypoints.map(wp => {
            const feature = new Feature({
              geometry: new Point(fromLonLat(wp)),
              id: i
            });
          ...
          const pathGeometry = new LineString(waypoints.map(wp => fromLonLat(wp)));
          const pathFeature = new Feature(pathGeometry);
          ...
          console.log(waypointsUntilCurrent);
          ...
          this.humidityIdwLayer.getSource().addFeatures(waypointsUntilCurrent.map(wp => {
            ...
            // get value in table at index wp.id
  ...
  methods: {
    ...
    updateLegend(layerType) {
      ...
      if (layerType === 'humidity') {
        ...
        gradient.style.background = 'linear-gradient(to right, lightblue, orange)';
