...
class Reader(IReader):
    ...
    def __init__(
        self, source_name: Union[str, int], batch_size: Optional[int] = None, dynamic_batch: Optional[bool] = False, **kwargs
    ) -> None:
        ...
        # open video stream
        self._video_stream = cv2.VideoCapture(int(self._name) if self._name.isdigit() else self._name)
    ...
    def _update_info(self) -> None:
        ...
        self._width = int(self._video_stream.get(cv2.CAP_PROP_FRAME_WIDTH) / 2)
        ...
        # update info
        self._info = {"name": self._name, "width": self._width, "height": self._height, "fps": self._fps, "backend": self._backend}
    ...
    def read_frame(self) -> Union[np.ndarray, None]:
        ...
        # get first half of the frame
        start = time.time()
        # get view of first half of the frame (no copy) with view() 
        first_half = frame.view()
        # set shape of the view to be half of the original frame
        first_half.shape = (frame.shape[0], frame.shape[1] // 2, frame.shape[2])
        # get view of second half of the frame (no copy) with view()
        second_half = frame.view()
        
        print("time taken to process: ", time.time() - start)
        self._frame_count += 0 if rotated_half is None else 1
        self._is_open = flag
        return rotated_half
