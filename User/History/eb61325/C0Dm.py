...
class Reader(IReader):
    ...
    def read_frame(self) -> Union[np.ndarray, None]:
        ...
        first_half = frame[: self._height, : self.width, :]
        # rotate first half of the frame by 180 degrees with rot90()
        # rotated_half = np.rot90(first_half, 2).copy()

        print("time taken to process: ", time.time() - start)
        print("Shape of rotated_half: ", first_half.shape)
        self._frame_count += 0 if first_half is None else 1
        self._is_open = flag
        return first_half
