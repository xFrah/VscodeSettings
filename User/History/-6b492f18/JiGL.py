    ...
    def update(self):
        ...
        for t in reversed(range(len(self.buffer.rewards))):
            delta = (
                ...
                + self.gamma * self.buffer.state_values[t + 1] * (1 - self.buffer.is_terminals[t])
                - self.buffer.state_values[t]
            )
        ...
        rewards = torch.tensor(advantages, dtype=torch.float32).to(device) + torch.tensor(
            self.buffer.state_values[:-1], dtype=torch.float32
        ).to(device)
