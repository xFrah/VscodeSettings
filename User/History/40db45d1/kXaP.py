...
def train():
    ...
    env_df = pd.read_csv("dataset/EURUSD_M1_env.csv", index_col=0, parse_dates=True)
    ################################
    ...
    max_ep_len = len(env_df)  # max timesteps in one episode
    ...
    ################ PPO hyperparameters ################
    update_timestep = 1e4  # update policy every n timesteps
    ...
    env = gym.make(
        "TradingEnv", df=env_df, positions=[-1, 0, 1], trading_fees=0.0003, windows=10
    )
    ...
    while time_step <= max_training_timesteps:
        ...
        for _ in tqdm(range(1, max_ep_len + 1)):
            ...
            if time_step % print_freq == 0:
                ...
                print(
                    "Episode : {} \t\t Timestep : {} \t\t Cumulative Reward : {} \t\t Episodic Length : {}".format(
                        i_episode,
                        time_step,
