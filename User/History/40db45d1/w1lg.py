...
hyperparameters = {
    ...
    "d_model": 19,  # Number of characteristics - 1
    "nhead": 4,  # Number of heads in the multi-head attention layers
    "nhid": 2048,  # Number of hidden units in the feedforward network model
...
for epoch in range(epochs):
    ...
    for episode in tqdm(range(len(data))):
        ...
        df = pd.DataFrame(data_day, columns=[
                          "Time", "Open", "High", "Low", "Close", "Volume", "Action","SMA","EMA","RSI","MACD","BBH","BBL", "datetime"])
        ...
        env = trading_env.make(
            ...
            feature_names=[
                ...
                "Low",
                "Close",
                "Volume","SMA","EMA","RSI","MACD","BBH","BBL"
        ...
        for i in range(len(data_day) - 1):
            ...
            if counter < 0:
                ...
                reward_batch = reward_batch.to(device=device)
                ...
                # Note that we use 'model(next_state).detach()' to make sure gradients are not backpropagated through the target.
                with torch.no_grad():
                    tensor = data_to_tensors(next_state).to(device=device)
                    target_q_value = reward + \
                        hyperparameters["gamma"] * \
                        torch.max(agent.model(tensor).detach())

                # Compute the predicted Q value by passing the current state through our model and selecting the Q value of the chosen action.
                tensor = data_to_tensors(state).to(device=device)
