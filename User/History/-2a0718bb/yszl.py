...
while True:
    ...
    if k == 13:
        ...
        print(markers)
        permutation, distance = find_shortest_path([field.frac_to_coord(*marker)[::-1] for marker in markers])
        print(permutation)
        print(f"Distance traveled: {distance * 111000, 2:.2f}m")

        for i, marker in enumerate(markers):
            coords = field.frac_to_coord(*marker)[::-1]
            folium.Marker(coords, popup=f"Waypoint {i}").add_to(fmap)
            # make line between permutation[i] and permutation[i+1]
