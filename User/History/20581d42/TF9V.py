...
def run_tracker_in_thread(source, padiglione, invert_flag):
    ...
    while True:
        ...
        if frame_counter % 1000 == 0:
            # save the frame to disk every 1000 frames
            cv2.imwrite(f"images/frame_{padiglione}_{frame_counter}.jpg", frame)
        ...
        for person, (first_appearance, init_position, last_position) in list(det_dict.items()):
            ...
            if datetime.datetime.now() - first_appearance > delta:  # has it disappeared?
                if side0 != side1:
                    v = 1 if side1 == "right" else -1
                    v *= -1 if invert_flag else 1
                    padiglione_dict[padiglione] += v
...
def main():
    ...
    try:
        client = mqtt.Client(client_id=conf["mqtt_user"], clean_session=True, userdata=None, protocol=mqtt.MQTTv311, transport="tcp")
        client.username_pw_set(conf["mqtt_user"], conf["mqtt_password"])
        client.on_connect = on_connect
        client.on_disconnect = on_disconnect
        client.connected_flag = False
        client.connect(conf["mqtt_host"], port=conf["mqtt_port"], keepalive=conf["mqtt_keepalive"])
        client.loop_start()
    except Exception as e:
        print(f"[ERROR] MQTT connection failed. {e}")

    while True:
        for pid in padiglione_dict:
            buf = padiglione_dict[pid]
            with open("padiglioni/" + pid + ".txt", "w") as f:
                f.write(str(buf))
            print(f"[INFO] Saved {pid}:{buf}")
            if client.connected_flag:
                try:
                    client.publish(f"pad{pid}", buf)
                    print(f"[LOG] Sent {pid}:{buf}")
                except Exception as e:
