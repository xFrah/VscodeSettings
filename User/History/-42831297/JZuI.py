...
def construct_mqtt_packet(
    ...
    packet.update({k: x for k, x in (("image", image), ("points", distances), ("people", people)) if x is not None})
    ...
    try:
        ...
        while True:
            ...
            if len(yolo_dataframes) > 0:
                try:
                    people_angles, image = yolo_dataframes[-1]
                except Exception as e:
                    print(f"Error getting people angles: {e}")
                    people_angles, image = None, None
