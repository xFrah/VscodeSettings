...
def get_angle_from_bbox(bbox, frame):
    ...
    x_center *= total_fov
    return x_center + angle_lower_limit_1
    ...
    while True:
        try:
            ...
            for frame in reader:
                ...
                for pred in preds:
                    bbox = pred.bbox
                    angle_ = get_angle_from_bbox(bbox, frame)
...
def commander():
    """
    This is the main thread, it gathers data from all the sensors and sends it to the mqtt broker.
    """
    try:
        ...
        while True:
            ...
            if len(yolo_dataframes) == 0:
                continue
            last_yolo_dataframe = yolo_dataframes[-1]
            people, image = decode_yolo_dataframe(lidar, slices, last_yolo_dataframe)
...
if __name__ == "__main__":
    ...
    lidar_addr = USB_Dispatcher(pyudev.Context()).get_tty_by_devicename("Lidar")
