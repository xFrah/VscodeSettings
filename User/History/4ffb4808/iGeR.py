...
def main(video_path, roi_path) -> None:
    ...
    while True:
        ...
        if success:
            ...
            if session_started:
                if (current_timestamp - last_feature_timestamp) > 10:
                    current_background = cv2.cvtColor(current_background, cv2.COLOR_BGR2RGB)
                    cropped = cv2.cvtColor(cropped, cv2.COLOR_BGR2RGB)

                    # Convert to Lab color space for a better color comparison
                    source_img_lab = cv2.cvtColor(source_img_cropped, cv2.COLOR_BGR2Lab)
                    reference_img_lab = cv2.cvtColor(reference_img_cropped, cv2.COLOR_BGR2Lab)

                    # Apply histogram matching
                    source_img_matched = match_histograms(source_img_lab, reference_img_lab)

                    # Convert back to BGR color space
                    source_img_matched_bgr = cv2.cvtColor(source_img_matched, cv2.COLOR_Lab2BGR)
