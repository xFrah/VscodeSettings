        ...
        if success:
            ...
            if session_started:
                if (current_timestamp - last_feature_timestamp) > 10:
                    ...
                    # convert to hsv
                    current_background = cv2.cvtColor(current_background, cv2.COLOR_RGB2HSV)
                    cropped = cv2.cvtColor(cropped, cv2.COLOR_RGB2HSV)

                    # make v channel equal
                    current_background[:, :, 2] = cropped[:, :, 2]

                    # convert back to rgb
                    current_background = cv2.cvtColor(current_background, cv2.COLOR_HSV2RGB)
                    cropped = cv2.cvtColor(cropped, cv2.COLOR_HSV2RGB)
