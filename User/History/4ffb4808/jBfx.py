        ...
        if success:
            ...
            if session_started:
                if (current_timestamp - last_feature_timestamp) > 10:
                    ...
                    current_background = cv2.cvtColor(current_background, cv2.COLOR_BGR2Lab)
                    cropped = cv2.cvtColor(cropped, cv2.COLOR_BGR2Lab)

                    # Apply histogram matching
                    cropped = exposure.match_histograms(cropped, current_background, multichannel=True)

                    # convert back to BGR
                    current_background = cv2.cvtColor(current_background, cv2.COLOR_Lab2BGR)
