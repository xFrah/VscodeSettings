...
def main(video_path, roi_path) -> None:
    ...
    while True:
        ...
        if success:
            ...
            cropped2 = roi.get_cropped_and_translated_image3(img)
            cropped2 = cv2.GaussianBlur(cropped2, (3 mà, 3), 0)

            if not session_started:
                background_subtractor.apply(cropped2)
            ...
            if session_started:
                if (current_timestamp - last_feature_timestamp) > 8:
                    ...
                    # cropped2 = cv2.resize(cropped, (cropped.shape[1] * 2, cropped.shape[0]))
                    # show current background and cropped image
                    cv2.imshow("Background", current_background)
                    cv2.imshow("Cropped2", cropped)
                    cv2.imshow("Cropped3", cropped2)
                    mask = change_detection.predict(change_detector, current_background, cropped2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--roi_path", help="path to roi file", default="ROI.csv")
