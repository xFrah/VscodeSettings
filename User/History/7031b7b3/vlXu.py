...
class LoadStreams:
    ...
    def __init__(self, sources="file.streams", img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.imgs[i] = cv2.rotate(self.imgs[i], cv2.ROTATE_180)
            # cut off the second half of the image
            self.imgs[i] = self.imgs[i][0 : self.imgs[i].shape[0], 0 : int(self.imgs[i].shape[1] / 2)]
    ...
    def update(self, i, cap, stream):
        ...
        while cap.isOpened() and n < f:
            ...
            if n % self.vid_stride == 0:
                ...
                # cut off the second half of the image
                im = im[0 : im.shape[0], 0 : int(im.shape[1] / 2)]
                if success:
                    self.imgs[i] = im
                else:
    ...
    def collate_fn4(batch):
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                im1 = F.interpolate(im[i].unsqueeze(0).float(), scale_factor=2.0, mode="bilinear", align_corners=False)[0].type(
                    im[i].type()
                )
                lb = label[i]
            else:
