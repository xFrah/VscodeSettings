...
def reward(history: dict):
    profit = np.log(history["portfolio_valuation", -1] / history["portfolio_valuation", -2])
    ...
    # argmin portfolio
    amin = np.argmin(history["portfolio_valuation"][am:])
    max_drawdown = np.log(history["portfolio_valuation"][am] / history["portfolio_valuation"][amin])
    max_drawdown_scale = -1

    print(f"profit: {profit}, trading_frequency_reward: {trading_frequency_reward}, max_drawdown: {max_drawdown}")

    # position holding time penalty

    return profit + ((trading_frequency_reward / max_e) * trading_frequency_reward_scale) + (max_drawdown * max_drawdown_scale)
