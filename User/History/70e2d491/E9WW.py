...
from thumbnail_utils import get_thumbnail_url, download_thumbnail, check_if_thumbnail_exists, url_to_file_name, create_image_with_label
...
class Movie:
    ...
    def cache_thumbnail(self, force: bool = False):
        ...
        if force:
            ...
            if path is not None:
                return path
            path = download_thumbnail(self.name, get_thumbnail_url(self.name))
            if path is not None:
                return path
            temp = create_image_with_label(self.name)
            # save it to temp_images_path
            temp_path = os.path.join(temp_images_path, self.name + ".png")
            cv2.imwrite(temp_path, temp)
            return os.path.join(relative_temp_images_path, self.name + ".png")
...
# get .streamlit folder
streamlit_config = os.path.join(user_path, ".streamlit", "config.toml")
# if config file doesn't exist, create it
if not os.path.exists(streamlit_config):
    print("[MU] Creating config file at " + streamlit_config)
    default_config = os.path.join(os.path.dirname(__file__), "config.toml")
    shutil.copyfile(default_config, streamlit_config)
else:
    print("[MU] Config file already exists at " + streamlit_config)

# clear temp_images
for file in os.listdir(temp_images_path):
    os.remove(os.path.join(temp_images_path, file))
    print(f"[MU] Removed {file} from {temp_images_path}")
