...
from thumbnail_utils import get_thumbnail_url, download_thumbnail, check_if_thumbnail_exists
...
class Movie:
    def __init__(self, name: str, path: str) -> None:
        self.name = name
        self.path = path
        print(f"Movie path for {self.name}: {self.path}")
    ...
    def get_config(self) -> dict[str, str]:
        ...
        if os.path.exists(self.config_file):
            ...
            print("Movie " + self.name + " does not have a config file at '" + self.config_file + "'.")
            return {}
    ...
    def has_config(self):
        ...
        if has_it:
            print("Movie " + self.name + " has a config file.")
        else:
            print("Movie " + self.name + " does not have a config file at '" + self.config_file + "'.")
        return has_it
...
class Movies:
    ...
    def scavenge(self):
        ...
        """
        print("Scavenging movies at " + self.video_folder_path)
    ...
    def get_movie_by_path(self, path: str):
        ...
        for movie in self.movies.values():
            if movie.cached_thumbnail is not None and movie.cached_thumbnail == path:
                print(f"1 - Found movie {movie.name} with path {path}")
                return movie
        for movie in self.movies.values():
            if movie.name == os.path.splitext(os.path.basename(movie.path))[0]:
                print(f"1 - Found movie {movie.name} with name {os.path.splitext(os.path.basename(movie.path))[0]}")
                return movie
        return None
