...
class Movies:
    ...
    def get_movie_by_path(self, path: str):
        ...
        """
        path = os.path.join(self.video_folder_path, path)
        for movie in self.movies.values():
            if os.path.join(cached_thumbnails_path, movie.cached_thumbnail if movie.cached_thumbnail is not None else "adsdfds") == path:
                return movie
        return None
