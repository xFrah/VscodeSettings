...
def construct_mqtt_packet(
    ...
    if people_angles is not None and slices is not None:
        packet["people"] = [(angle, lidar.get_distance_at_angle(slices, angle)[0]) for angle, _ in people_angles]
    if tof1 is not None:
...
def yolo_run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if counter == 10:
                print(f"Yolo >> FPS: {10 / (time.time() - start_time)}")
                start_time = time.time()
                counter = 0
            counter += 1
        ...
        for people_angles, frame in local_detections:
            client.publish(
                ...
                construct_mqtt_packet(
                    lidar,
                    base64.b64encode(tof1_frame.tostring()).decode("utf-8"),
                    base64.b64encode(tof2_frame.tostring()).decode("utf-8"),
                    distances,
                    slices,
                    people_angles,
                    angle_lower_limit_1,
            ...
            # np.fromstring(base64.b64decode(encoded), dtype=np.uint8).reshape(480, 1280, 3)
            c += 1
            if c % 10 == 0:
                # print fps
