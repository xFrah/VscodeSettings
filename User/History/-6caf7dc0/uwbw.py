...
def commander():
    img_shape = (480, 1280)
    detect_mqtt_broker()
    lidar = LIDAR.Lidar(port=lidar_addr[0], slices=True, buffer_size=1)
    lidar.active(True)
    print("Lidar active")
    time.sleep(1)
    tof1 = Tof(tof1_addr, background_shape=img_shape, matrix_path="COM8-M.npy")
    print("TOF1 active")
    time.sleep(1)
    tof2 = Tof(tof2_addr, background_shape=img_shape, matrix_path="COM12-M.npy")
    print("TOF2 active")
    start_time = time.time()
    # create mqtt client
    ...
    while True:
        ...
        # polisher(local_detections, detections_polished, slice_to_r_d)
        ...
        people_angles, frame = local_detections[-1] if len(local_detections) > 0 else ([], None)
        if frame is not None and distances is not None:
            client.publish(
                "humidity/",
...
def main():
    ...
    retries = 0
    while not tof1_addr or not tof2_addr or not lidar_addr:
        usb_disp.unbind_all()
        usb_disp.bind_all()
        tof1_addr = usb_disp.get_tty_by_devicename("TOF1")
        tof2_addr = usb_disp.get_tty_by_devicename("TOF2")
