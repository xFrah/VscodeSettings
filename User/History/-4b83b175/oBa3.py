    ...
    def _thread(self):
        try:
            ...
            self._stream = self._p.open(
                format=self._p.get_format_from_width(self.WIDTH), channels=self.CHANNELS, rate=self.RATE, input=True, frames_per_buffer=self.CHUNK
            )
            while self._running:
                st = time.time()

                bytes_data = self._stream.read(self.CHUNK)
