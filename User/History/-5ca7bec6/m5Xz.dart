...
class GestisciContenuto extends StatefulWidget {
  final Map<String, dynamic> jsonData;
  const GestisciContenuto({Key? key, required this.jsonData}) : super(key: key);

  @override
  _GestisciContenutoState createState() => _GestisciContenutoState();
}

class _GestisciContenutoState extends State<GestisciContenuto> {
  List<MapEntry<String, List<String>>> nonEmptyEntries = [];

  @override
  void initState() {
    super.initState();
    // Iterate over the entries of the map jsonData["contents"]
    widget.jsonData["contents"].forEach((key, value) {
      // If the value is not an empty list, if it is a list of strings cast it, add it to the nonEmptyEntries list
      if (value is List && value.isNotEmpty) {
        nonEmptyEntries.add(MapEntry(key, value.cast<String>()));
      }
    });
  }
        ...
        children: [
          LayoutBuilder(
            builder: (context, constraints) {
              final halfScreenWidth = constraints.maxWidth;
              return Container(
                width: halfScreenWidth,
                child: Image.asset('assets/4k.378.png', fit: BoxFit.fitWidth),
              );
            },
          ),
          Expanded(
            child: ListView.builder(
              itemCount: nonEmptyEntries.length, // <-- Updated this line
              itemBuilder: (context, index) {
                // Randomly select an icon from the provided list
                final icons = [
                  Icon(Icons.vaccines),
                ...
                final MapEntry entry =
                    nonEmptyEntries[index]; // <-- Updated this line

                final id = entry.value[0];

                return _listItem(randomIcon, entry.key,
                    widget.jsonData["expirations"][id], id);
              },
            ),
          )
        ],
  ...
  Widget _listItem(Icon iconData, String item_type, String id, String id2) {
    // check if id is in jsonData["missing"]
    // capitalize first letter of item_type and replace underscores with spaces
    item_type = item_type[0].toUpperCase() +
        item_type.substring(1).replaceAll("_", " ");
    final bool isMissing = widget.jsonData["missing"].contains(id2);
    Color iconColor = isMissing ? Colors.orange : Colors.green;
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            Icon(
              iconData.icon, // The actual IconData
              color: iconColor, // Using the determined color
              size: 30.0, // Ensuring a size
            ),
            SizedBox(width: 16.0),
            Expanded(
              child: Column(
            ...
            Icon(Icons.arrow_forward_ios, color: const Color(0xff27312f)),
          ],
        ),
      ),
    );
