...
class UHF:
    ...
    def receiver(self):
        ...
        try:
            ...
            while True:
                ...
                data_buffer += res
                # print how many bytes are in os uart buffer
            ...
            print("[RFID] Error in receiver thread:", e)
    ...
    def deep_scan(self, timeout=30, careless=False) -> dict[str, int]:
        """
        ...
        step_n = 0
        max_n = 0
        while time.time() - start < timeout and (not sec_obj.authed or careless):
            ...
            for epc, rssi in tags:
                ...
                if hashable not in permasaw:
                    permasaw[hashable] = [rssi]
                else:
                    permasaw[hashable].append(rssi)
            ...
            if other:
                for packet in other:
                    if packet._data["msg"] != "Inventory fail. No tag response or CRC error" and packet._data["msg"] != "Unknown response":
                        print(packet)
        self.stop_scan()
    ...
    def get_data(self, start=None):
        ...
        for packet in self.packets.copy():
            ...
            if start is not None and packet.timestamp < start:
                ...
                remove_packet(packet)
                tags.append((packet["epc"], packet["rssi"]))
            else:
                remove_packet(packet)
                other_packets.append(packet)
        return tags, other_packets

    def scan(self, wait=0.1):
        """
