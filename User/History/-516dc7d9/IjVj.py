...
if __name__ == "__main__":
    ...
    R1, R2, P1, P2, Q, roi1, roi2 = cv.stereoRectify(cmtx0, dist0, cmtx1, dist1, (640, 480), R1, T1)

    # use initUndistortRectifyMap to get maps
    mapL1, mapL2 = cv.initUndistortRectifyMap(cmtx0, dist0, R1, P1, (640, 480), cv.CV_32FC1)
    mapR1, mapR2 = cv.initUndistortRectifyMap(cmtx1, dist1, R2, P2, (640, 480), cv.CV_32FC1)
    ...
    while True:
        ...
        # use remap to rectify images
        imgL_u = cv.remap(imgL, mapL1, mapL2, cv.INTER_LINEAR)
        imgR_u = cv.remap(imgR, mapR1, mapR2, cv.INTER_LINEAR)
        ...
        beta = 1 - alpha
        gamma = 0
        imgL_u = cv.addWeighted(imgL_u, alpha, imgL, beta, gamma)
        imgR_u = cv.addWeighted(imgR_u, alpha, imgR, beta, gamma)
