...
class Request:
    ...
    def __init__(self, app, client_addr, method, url, http_version, headers, body=None, stream=None, sock=None):
    ...
    def create(app, client_stream, client_addr, client_sock=None):
        ...
        return Request(app, client_addr, method, url, http_version, headers, stream=client_stream, sock=client_sock)
        ...
        if len(urlencoded) > 0:
            if isinstance(urlencoded, str):
                for kv in [pair.split("=", 1) for pair in urlencoded.split("&") if pair]:
                    data[urldecode_str(kv[0])] = urldecode_str(kv[1]) if len(kv) > 1 else ""
            elif isinstance(urlencoded, bytes):  # pragma: no branch
                for kv in [pair.split(b"=", 1) for pair in urlencoded.split(b"&") if pair]:
                    data[urldecode_bytes(kv[0])] = urldecode_bytes(kv[1]) if len(kv) > 1 else b""
        return data
...
class Response:
    ...
    def set_cookie(self, cookie, value, path=None, domain=None, expires=None, max_age=None, secure=False, http_only=False):
...
class Microdot:
    ...
    def route(self, url_pattern, methods=None):
        ...
        def decorated(f):
            self.url_map.append(([m.upper() for m in (methods or ["GET"])], URLPattern(url_pattern), f))
            return f
    ...
    def mount(self, subapp, url_prefix=""):
        ...
        for methods, pattern, handler in subapp.url_map:
            self.url_map.append((methods, URLPattern(url_prefix + pattern.url_pattern), handler))
    ...
    def handle_request(self, sock, addr):
        if Request.socket_read_timeout and hasattr(sock, "settimeout"):  # pragma: no cover
