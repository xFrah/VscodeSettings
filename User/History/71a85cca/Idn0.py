...
class Pentagramma:
    def __init__(self):
        ...
        # dictionary for converting letter note to classic note
        self.letter_to_classic = {"G": "sol", "F": "fa", "E": "mi", "D": "re", "C": "do", "B": "si", "A": "la"}
    ...
    def draw_note(self, note, img, end=False):
        # get y coordinate of note
        # get light blue color
        light_blue = (56, 146, 255)

        y = self.offset + (self.interspace * self.notes[note])
        x = self.x_offset if not end else img.shape[1] - self.x_offset + 1
        # draw note
        cv2.ellipse(img, (x, y), (int(self.interspace * 1.2), int(self.interspace / 1.2)), 0, 0, 360, (0, 0, 0) if not end else light_blue, -1)
        return img
    ...
    for img, note in pentagramma:
        ...
        while True:
            ...
            if pitch is not None:
                ...
                if "♯" in pitch:
                    pitch = pitch.replace("♯", "")
                pitch = pentagramma.letter_to_classic[pitch]
                pentagramma.draw_note(pitch, temp, end=True)
