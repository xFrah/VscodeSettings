...
def land_at_waypoint_mission(drone: Drone, lat, lon, alt):
    ...
    for _ in range(20):
        ...
        res = drone.get_packet("MISSION_ACK", no_older_than=how_many_seconds_since_start, pop=True, timeout=0.5, raise_exception=False)
        if res is None:
            continue
        if res.type:
    ...
    for _ in range(20):
        ...
        how_many_seconds_since_start = time.time() - start
        res = drone.get_packet("MISSION_REQUEST", no_older_than=how_many_seconds_since_start, pop=True, timeout=0.5, raise_exception=False)
        if res:
            break
    if not res:
    ...
    for _ in range(20):
        ...
        how_many_seconds_since_start = time.time() - start
        res = drone.get_packet("MISSION_ACK", no_older_than=how_many_seconds_since_start, pop=True, raise_exception=False, timeout=1)
        if res is None:
            continue
        if res.type:
