...
def do_command(drone: Drone, cmd: List[int], timeout: int = 10, retries: int = 5, failed_rebounce: float = 5):
    ...
    for i in range(retries):
        ...
        while True:
            msg = drone.get_packet("COMMAND_ACK", no_older_than=1, pop=True, raise_exception=False, timeout=timeout)
            if msg is None:
                time.sleep(0.2)
                break
...
def land_at_waypoint_mission(drone: Drone, lat, lon, alt):
    ...
    # clear mission
    for _ in range(40):
        print("Mission clear ACK: ", flush=True)
        drone.master.mav.mission_clear_all_send(
            drone.master.target_system,
            drone.master.target_component,
            mavutil.mavlink.MAV_MISSION_TYPE_MISSION,
        )
        how_many_seconds_since_start = time.time() - start
        res = drone.get_packet("MISSION_ACK", no_older_than=how_many_seconds_since_start, pop=True, timeout=0.1, raise_exception=False)
        if res is None:
            continue
        if res.type:
    ...
    print("Mission count ACK: ", flush=True)
    for _ in range(40):
        failed = False
        for _ in range(40):
            ...
            how_many_seconds_since_start = time.time() - start
            res = drone.get_packet("MISSION_REQUEST", no_older_than=how_many_seconds_since_start, pop=True, timeout=0.1, raise_exception=False)
            if res:
                break
        if not res:
            raise MissionError("couldn't get mission request")
        print("Success")

        print("Mission send ACK: ", flush=True)
        for _ in range(40):
            ...
            how_many_seconds_since_start = time.time() - start
            res = drone.get_packet("MISSION_ACK", no_older_than=how_many_seconds_since_start, pop=True, raise_exception=False, timeout=0.1)
            if res is None:
                continue
            if res.type:
                failed = True
