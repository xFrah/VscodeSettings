...
class _WifiCredentialsScreenState extends State<WifiCredentialsScreen> {
  ...
  Future<void> wifiSetup(String newSsid, String newPassword) async {
    String command = "command:wifi_save+$newSsid+$newPassword";
    // get list of integers from command string
    final commandInt = command.codeUnits;
            ...
            for (var characteristic in service.characteristics) {
              if (characteristic.uuid.toString() == CMD_RX_CHAR_UUID) {
                ...
                stream = characteristic.value.listen((value) async {
                  ...
                  if (handleReceivedValue(response, command)) {
                ...
                await characteristic.write(commandInt);
  ...
  bool handleReceivedValue(String lastKnownCommandStr, String commandToSend) {
    ...
    if (command == commandToSend) {
      if (result == "1") {
        print("[BLE] Successfully sent wifi setup command");
        return true;
      }
      print("[BLE] Couldn't send wifi setup command");
      return false;
    }
