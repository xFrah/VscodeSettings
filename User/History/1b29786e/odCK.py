...
for region, constraints in constraints.items():
    for constraint in constraints:
        if region_colors[region].color == region_colors[constraint].color:
            print(f"Constraint not satisfied: {region} and {constraint} have the same color")
            b = False
        else:
            print(
                f"Constraint satisfied: {region}, {constraint} = {rgb_to_color[region_colors[region].color]}, {rgb_to_color[region_colors[constraint].color]}"
            )
