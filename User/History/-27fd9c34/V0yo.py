...
class BLEPeripheral:
    ...
    def get_mac_address(self) -> str:
        """Get mac address of ble peripheral."""
        mac = "".join(["{:02X}".format(x) for x in self._ble.config("mac")])
        ...
        while True:
            try:
                ...
                if not self._connections:
                    self.notify_wifi_status(WMS.update_status())
                    continue
                    ...
                    for command, (args, whole, value_handle) in cmds:
                        try:
                            if command == b"wifi_save":
                                if len(args) != 2:
                                    print("[BLE] Wrong number of arguments for wifi_save", args)
                                    continue
