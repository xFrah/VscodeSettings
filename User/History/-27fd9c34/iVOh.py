...
from wifi_manager import get_wifi_credentials, write_wifi_credentials
...
class BLEPeripheral:
    ...
    def mini_ble_thread(self):
        while True:
            if ble._connections:
                return ble.notify_wifi_status(WMS.update_status())
            if len(ble.commands) > 0:
                cmds = list(ble.commands.items())
                ble.commands.clear()
                for command, (args, whole, value_handle) in cmds:
                    try:
                        if command == b"wifi_save":
                            ...
                            ble.confirm_result(True, value_handle, whole)
                            ...
                            # TODO acquire all locks, because this is in another thread
                            ble.confirm_result(True, value_handle, value)
                            machine.reset()
                        else:
                ...
                if value.startswith(b"command:"):
                    try:
                        ...
                        if len(parsed) == 3:
                            ...
                            print("[BLE] Received unknown command", value)
                    except Exception as e:
