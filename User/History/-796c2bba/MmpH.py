...
class MQTT_base:
    ...
    async def _connect(self, clean):
        ...
        try:
            ...
            if e.args[0] not in BUSY_ERRORS:
                raise
        await asyncio.sleep_ms(_DEFAULT_MS)
        self.dprint("[MQTT_DEBUG] Connecting to broker.")
        if self._ssl:
            import ussl
        ...
        self.dprint("[MQTT_DEBUG] Connected to broker.")  # Got CONNACK
    ...
    def close(self):  # API. See https://github.com/peterhinch/micropython-mqtt/issues/60
        ...
        try:
            ...
            self.dprint("[MQTT_DEBUG] Wi-Fi not started, unable to disconnect interface")
...
class MQTTClient(MQTT_base):
    ...
    async def wifi_connect(self, quick=False):
        ...
        if not quick:  # Skip on first connection only if power saving
            ...
            self.dprint("[MQTT_DEBUG] Checking WiFi integrity.")
            for _ in range(5):
                if not s.isconnected():
                    raise OSError("Connection Unstable")  # in 1st 5 secs
                await asyncio.sleep(1)
            self.dprint("[MQTT_DEBUG] Got reliable connection")
        ...
        try:
            if not self._has_connected and self._clean_init and not self._clean:
                ...
                try:
                    ...
                    pass
                self.dprint("[MQTT_DEBUG] Waiting for disconnect")
                await asyncio.sleep(2)  # Wait for broker to disconnect
                self.dprint("[MQTT_DEBUG] About to reconnect with unclean session.")
    ...
    async def _keep_alive(self):
        while self.isconnected():
            ...
            if pings_due >= 4:
                self.dprint("[MQTT_DEBUG] Reconnect: broker fail.")
                break
    ...
    async def _memory(self):
        while True:
            ...
            gc.collect()
            self.dprint("[MQTT_DEBUG] RAM free %d alloc %d", gc.mem_free(), gc.mem_alloc())
        ...
        while self._has_connected:
            if self.isconnected():  # Pause for 1 second
                ...
                if not self._has_connected:  # User has issued the terminal .disconnect()
                    self.dprint("[MQTT_DEBUG] Disconnected, exiting _keep_connected")
                    break
                try:
                    await self.connect()
                    # Now has set ._isconnected and scheduled _connect_handler().
                    self.dprint("[MQTT_DEBUG] Reconnect OK!")
                except OSError as e:
                    self.dprint("[MQTT_DEBUG] Error in reconnect. %s", e)
                    # Can get ECONNABORTED or -1. The latter signifies no or bad CONNACK received.
                    self._close()  # Disconnect and try again.
                    self._in_connect = False
                    self._isconnected = False
        self.dprint("[MQTT_DEBUG] Disconnected, exited _keep_connected")
