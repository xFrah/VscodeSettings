...
def click_event(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if len(coordinates) == 4:
            ...
            # create roi object and display get_cropped_and_translated_image3
            roi = ROI("ROI.csv")
            cropped_image = roi.get_cropped_and_translated_image3(frame)
            cv2.imshow("Cropped Image", cropped_image)
            cv2.waitKey(0)
            # cv2.destroyAllWindows()

# Read the first frame from the video file
cap = cv2.VideoCapture('media/Sedili.mp4')  # Replace with your video path
ret, frame = cap.read()
if not ret:
