...
import numpy as np
import cv2
from helpers import order_points, sort_coordinates, find_rectangle_bases_points
...
class FootballField(Land):
    ...
    def __init__(self, vertices: List[Tuple[float, float]]) -> None:
        ...
        """
        assert len(vertices) == 4, "A football field must have 4 vertices."
        vertices = sort_coordinates(vertices)
        unk_and_vertices = find_rectangle_bases_points(vertices)
        if len(unk_and_vertices) == 2:
            _, vertices = unk_and_vertices
        else:
            vertices = unk_and_vertices
