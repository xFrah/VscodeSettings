...
def normalize_data_list(data_list):
    ...
    return [(0.5 + (p - start) / (2.0 * max_deviation + 0.000001))
            for p in data_list]
...
def get_slice_at_index(index, batch_size):
    ...
    prediction_slice = normalize_data_list(df.iloc[index: label_index + 3]["Close"].values)
...
def validate_training(df_val, input_size, train_losses, val_losses, loss_function, model, device):
    ...
    label, df_slice, _, _ = get_slice_at_index(val_idx, input_size)
