...
class Time2Vec(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.linear = nn.Linear(input_dim, output_dim // 2, dtype=torch.float64)
        self.periodic = nn.Linear(input_dim, output_dim // 2, dtype=torch.float64)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):  # x: (S, 1, E)
        ...
        position = torch.arange(0, S, dtype=torch.float).unsqueeze(1)
...
class TransformerModel(nn.Module):
    def __init__(self, ninp, nhead, nhid, nlayers, noutput, dropout=0.5):
        ...
        self.decoder = nn.Linear(nhid, noutput, dtype=torch.float64)
