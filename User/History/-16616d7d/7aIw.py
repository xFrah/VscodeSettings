...
class TransformerModel(nn.Module):
    ...
    def forward(self, src):  # src: (S, 1, E)
        ...
        # turn (S, 1, E) into (1, S, E)
        src = src.transpose(0, 1) # Output: (1, S, E)
        output = self.transformer_encoder(src)  # Output: (1, S, E)
        output = self.decoder(output)  # Output: (1, S, 1)
        # turn (1, S, 1) into (S, 1)
        output = output.transpose(0, 1).squeeze(2)  # Output: (S, 1)
        return output
...
class StockPredictionModel2(nn.Module):
    def __init__(self, d_model, nhead, num_layers, num_classes):
        ...
        self.classification = ClassificationLayer(d_model + 1, num_classes)
