...
class Device {
    ...
    class Device {
        ...
        public function receive($debug = true) {
            $response = null;
            $attempt = 1;
            while (empty($response) && $attempt <= 3) {
                try {
                    if ($debug) {
                        $this->debug("Attempt $attempt\n");
                    }
                    $response = $this->connector->receive($this->ioTimeout, $this->bufferSize);
                } catch (\Exception $e) {
                    $this->debug($e->getMessage());
            ...
            if (empty($response)) {
                throw new \Exception("Failed to receive message after 3 attempts.");
            }
            if ($debug) {
    ...
    public function receive($length = 0, $debug = true) {
        ...
        if ($message === false) {
            ...
            if (!$message->isAck() && !$message->isAsync()) {
                try {
                    $this->send(new \POS\Ingenico\iSelf\Messages\Ack, false);
                } catch (\Exception $e) {
                    throw new \Exception($e->getMessage());
                }
            }
