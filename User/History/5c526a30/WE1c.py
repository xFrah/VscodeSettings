...
class adxl345:
    def __init__(self, scl, sda, cs):
        ...
        for s in slv:
            ...
            if buf[0] == 0xE5:
                self.slvAddr = s
                print("[ADXL345] Device found")
                break
        if self.slvAddr is None:
            print("[ADXL345] Device not found")
            return
    ...
    def is_connected(self):
        return self.slvAddr is not None
    ...
    def writeByte(self, addr, data):
        if self.slvAddr is None:
            print("[ADXL345] Can't write byte, device is not connected.")
            return
        d = bytearray([data])
    ...
    def readByte(self, addr):
        if self.slvAddr is None:
            print("[ADXL345] Can't read byte, device is not connected.")
            return
