        ...
        if vl53.data_ready():
            ...
            if not movement:
                ...
                if len(asd) == 0 and ((now := datetime.datetime.now()) - start).total_seconds() > 0.3:
                    ...
                    rect, diff = get_segmentation_from_buffer(camera_target_frame, background, camera_target_frame_index, buffer)
                    ...
                    if len(asd) > 0:
                        tof_buffer[datetime.datetime.now()] = (data[:16], sum(asd) / len(asd))
                    count += 1
            ping(thread)
            count1 += 1
            if count1 == 100:
