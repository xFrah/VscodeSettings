...
def get_segmentation_from_buffer(camera_target_frame, background, camera_target_frame_index, buffer):
    rect, diff, biggest = helpers.get_diff(camera_target_frame, background)
...
def main():
    ...
    while True:
        if vl53.data_ready():
            ...
            if not movement:
                if len(asd) > 0 and (datetime.datetime.now() - last).total_seconds() > 1:
                    camera.shoot()
                ...
                if len(asd) == 0 and ((now := datetime.datetime.now()) - start).total_seconds() > 0.3:
                    ...
                    rect, diff, biggest = get_segmentation_from_buffer(camera_target_frame, background, camera_target_frame_index, buffer)
                    if (rect is not None) and (diff is not None) and (biggest is not None):
                        x, y, w, h = biggest
            ...
            count1 += 1
            if count1 == 500:
                print(f"[INFO] FPS: {count1 / (datetime.datetime.now() - start1).total_seconds()}")
                start1 = datetime.datetime.now()
                count1 = 0
