...
markers = None
json_markers: list[JsonWaypoint] = None
status = "TRAVELING"
i = 0
...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def send_data(self):
        ...
        if json_markers:
            msg["markers"] = [marker.to_json() for marker in json_markers]
            # print([marker.to_json() for marker in json_markers])
...
while True:
    ...
    for waypoint in waypoints_list:
        ...
        status = "MEASURING"
        time.sleep(5)
        json_markers[i].humidity = int(40 + (random() * 5))
        json_markers[i].temperature = int(15 + (random() * 2))
        i += 1
