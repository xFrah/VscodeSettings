...
export function correctHeadAndTail(startDateTime: string, endDateTime: string, overtimeMap: Map<string, number>, occurrences: Record<number, number>): Record<number, number> {
    ...
    console.log("Task start and end time", start, end, minutesLate, calculateHourPercentage(60 - start.getMinutes()), end.getMinutes(), calculateHourPercentage(end.getMinutes()), "Occurrences", occurrences)
