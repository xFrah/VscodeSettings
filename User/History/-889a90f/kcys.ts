...
export function correctHeadAndTail(startDateTime: string, endDateTime: string, overtimeMap: Map<string, number>, occurrences: Record<number, number>): Record<number, number> {
    ...
    console.log("Task start and end time", start, end, minutesLate, calculateHourPercentage(minutesLate), minutesOvertime, calculateHourPercentage(end.getMinutes()), "Occurrences", occurrences)
