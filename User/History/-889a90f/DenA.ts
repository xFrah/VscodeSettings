...
export function correctHeadAndTail(startDateTime: string, endDateTime: string, overtimeMap: Map<string, number>, occurrences: Record<number, number>): Record<number, number> {
    ...
    // get new date with minutes set to 0
    const newEnd = new Date(end);
    newEnd.setMinutes(minutesLate);
    // get absolute difference between new end date and end date
    const difference = Math.abs(end.getTime() - newEnd.getTime()) / 60000;
    // const difference = (end.getTime() - newEnd.getTime()) / 60000;
    if (difference === 0) {
        occurrences[lastElement] += calculateHourPercentage(minutesLate);
    } else {
