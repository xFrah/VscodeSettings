...
export function correctHeadAndTail(startDateTime: string, endDateTime: string, overtimeMap: Map<string, number>, occurrences: Record<number, number>): Record<number, number> {
    ...
    // get difference between new end date and end date
    const difference = (end.getTime() - newEnd.getTime()) / 60000;
    if (difference === 0) {
        occurrences[firstElement] += calculateHourPercentage(difference);
    } else {
        
    }
    if (minutesOvertime !== 0) {
        occurrences[lastElement] += calculateHourPercentage(minutesOvertime);
    } else {
