...
export function correctHeadAndTail(startDateTime: string, endDateTime: string, overtimeMap: Map<string, number>, occurrences: Record<number, number>, i: number): Record<number, number> {
    ...
    console.log("Task start and end time", start, end, 60 - start.getMinutes(), calculateHourPercentage(60 - start.getMinutes()), end.getMinutes(), calculateHourPercentage(end.getMinutes()), "Occurrences", occurrences)
...
export async function workerDashboard(parent, args, contextValue) {
    ...
    tasks.forEach((task) => {
        ...
        const overtimeMap = calculateHourlyOvertimePercentage(task?.start_time, task?.end_time, WORKER_TYPE === "EMPLOYEE", i);
        let tasksOvertime = countOvertimeOccurrences(overtimeMap);

        tasksOvertime = correctHeadAndTail(task?.start_time, task?.end_time, overtimeMap, tasksOvertime, i);
