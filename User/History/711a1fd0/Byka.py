...
class ROI:
    ...
    def get_cropped_and_translated_image3(self, image):
        original_height, original_width = image.shape[:2]
        # make 256x1024 blank image
        blank_image = np.zeros((256, 1024, 3), dtype=np.uint8)
        ...
        M = cv2.getPerspectiveTransform(roi_polygon_adjusted, np.array([[0, 0], [1024, 0], [1024, 256], [0, 256]], np.float32))
        warped_image = cv2.warpPerspective(image, M, (1024, 256))
