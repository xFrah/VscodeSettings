...
class ROI:
    ...
    def get_masked_image(self, image):
        # Create a mask for the ROI within the image
        mask = np.zeros(image.shape[:2], dtype=np.uint8)
        roi_polygon = np.array(self.roi_polygon, np.int32)
    ...
    def get_cropped_image(self, image):
        ...
        roi_polygon_adjusted = [(x - int(xmin), y - int(ymin)) for x, y in self.roi_polygon]
