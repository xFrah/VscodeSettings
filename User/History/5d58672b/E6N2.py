...
def dl(symbol: str):
    ...
    df["close"] = df["close"] - (df["close"].min() / 2)

    df["feature_close"] = df["close"]  # close price
    # df["feature_volume"] = df["volume"]  # volum
    # make feature_volume as the normalized volume, taking max and min
    df["feature_volume"] = (df["volume"] - df["volume"].min()) / (df["volume"].max() - df["volume"].min())
    ...
    # add b%
    df["feature_bb_perc"] = (df["close"] - df["bb_low"]) / (df["bb_high"] - df["bb_low"])
