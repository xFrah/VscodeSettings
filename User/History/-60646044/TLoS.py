...
particle_format = 'iiibxi' # Structure format for Particle
...
def receive_and_parse_packets(sock):
    ...
    while True:
        ...
        while True:
            ...
            # Extract and process the particle data
            particles_data = buffer[length_offset + length_size:packet_end - footer_size]
            for i in range(0, length, particle_size):
                particle = struct.unpack(particle_format, particles_data[i:i + particle_size])
                # print(f"Particle: ID={particle[0]}, X={particle[1]}, Y={particle[2]}, Walker={chr(particle[3])}, Y_Index={particle[4]}")
                current_packet[particle[0]] = Particle(particle[0], particle[1], particle[2], particle[3], particle[4])

            # do it with dictionary comprehension
