import os
import cv2
import imdb
import numpy as np
import requests

ia = imdb.IMDb()
print("Loading IMDB database...")


def get_thumbnail_url(movie_name):
    search_movie = ia.search_movie(movie_name)

    if len(search_movie) == 0:
        return None

    url = search_movie[0].data["cover url"]
    try:
        url = url[: url.rindex("@") + 1]
    except ValueError:
        return None
    return url


def download_thumbnail(movie_name, url):
    if url is None:
        return None
    response = requests.get(url)
    if response.status_code == 200:
        filename = movie_name + ".png"
        path = os.path.join("frontend/public", filename)
        with open(path, "wb") as file:
            file.write(response.content)
        return filename
    else:
        return None


def check_if_thumbnail_exists(movie_name):
    filename = movie_name + ".png"
    path = os.path.join("frontend/public", filename)
    return filename if os.path.exists(path) else None


def create_image_with_label(width, height, label_text):
    image = np.ones((height, width, 3), dtype=np.uint8) * 255  # Create a white image

    # Determine the dimensions of the label rectangle
    label_font = cv2.FONT_HERSHEY_SIMPLEX
    label_scale = 1.0
    label_thickness = 2
    (label_width, label_height), _ = cv2.getTextSize(label_text, label_font, label_scale, label_thickness)

    # Calculate the position of the label
    label_x = int((width - label_width) / 2)  # Centered horizontally
    label_y = int(0.7 * height)  # 0.7 of the image height

    # Check if the label overflows out of the image
    if label_y + label_height > height:
        label_y = height - label_height  # Adjust the label position

    # Draw the label rectangle and text on the image
    cv2.rectangle(image, (label_x, label_y), (label_x + label_width, label_y + label_height), (0, 0, 0), -1)
    cv2.putText(image, label_text, (label_x, label_y + label_height - 10), label_font, label_scale, (255, 255, 255), label_thickness)

    return image


# Example usage
image_width = 800
image_height = 600
label_text = "Hello, OpenAI!"

image = create_image_with_label(image_width, image_height, label_text)

cv2.imshow("Image", image)
cv2.waitKey(0)
cv2.destroyAllWindows()
