...
def predict(model, image1_path, image2_path, device, input_size):
    ...
    augs.append(T.Lambda(lambda img: img.convert("RGB")))
    augs.append(T.ToTensor())
    augs.append(T.Normalize(mean=mean, std=std))
    transform = T.Compose(augs)

    # Convert and preprocess images
    image1 = transform(image1).unsqueeze(0).to(device)  # Add batch dimension and send to device
...
def main(args):
    ...
    mask = predict(model, r"C:\Users\fdimo\Desktop\surveillance_module\CACHE\v1710943049.49-bg.png", r"C:\Users\fdimo\Desktop\surveillance_module\CACHE\v1710943092.75-bg.png", device, args.input_size)
