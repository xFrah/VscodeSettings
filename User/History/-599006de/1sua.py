...
def predict(model, image1_path, image2_path, device, input_size):
    ...
    with torch.no_grad():
        ...
        mask_pred = mask_pred.cpu().numpy().squeeze().astype(np.uint8) 
...
def main(args):
    ...
    mask = predict(model, "opt/Dataset/pcd/pcd/TSUNAMI/t0/00000000.jpg", "opt/Dataset/pcd/pcd/TSUNAMI/t1/00000000.jpg", device, args.input_size)
