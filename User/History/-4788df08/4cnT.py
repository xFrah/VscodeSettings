...
class WiFiManager:
    def __init__(self):
        ...
        self.status = 3  # initial status (not yet initialized)

    def connect_to_wifi(self, retries=5, retry_timeout=5, fallback=None):
        """
        Connect to Wi-Fi by automatically retrieving credentials from file.
        """
        ssid, password = get_wifi_credentials()
        print("[WIFI] Connecting to Wi-Fi...")
        self.status = 2  # status: trying to connect
        ...
        while not self.sta_if.isconnected() and not self.stop_retry:  # wait for it to connect or stop flag to be set
            ...
            if attempt > retries:
                ...
                self.status = 0  # status: not connected
        ...
        self.status = 1  # status: connected
        return True
    ...
    def update_status(self):
        ...
        3 = not initialized.
        """
        self.status = 1 if self.sta_if.isconnected() else 0
        return self.status
