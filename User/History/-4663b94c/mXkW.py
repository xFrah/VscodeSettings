...
def write_file(filtered_boxes, start_time_text, start_time, sessions) -> None:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if success:
            ...
            if args.skip_frames != 0:
                ...
                if frame_count2 % args.skip_frames != 0:
                    continue
            ...
            if boxes is not None:
                for box, score, cl in zip(boxes, scores, classes):
                    ...
                    if cl == 0 and score > args.confidence:
                        if not session_started:
                            session_started = current_timestamp + 0
            ...
            if session_started:
                ...
                if (current_timestamp - last_feature_timestamp) > 8:
                    ...
                    background_subtractor.setDetectShadows(True)
                    print("Session ended")
                    sessions.append({"start": session_started, "end": current_timestamp})
                    session_started = False
            else:
                if last_cut + normal_cut_time < current_timestamp:
                    write_file(
                        filtered_boxes=filtered_boxes,
                        start_time_text=video_saver.start_time_text,
                        start_time=video_saver.start_time,
                        sessions=sessions,
                    )
    ...
    if close_last_video:
        write_file(filtered_boxes=filtered_boxes, start_time_text=video_saver.start_time_text, start_time=video_saver.start_time, sessions=sessions)
