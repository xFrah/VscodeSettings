...
class VLC_Instance:
    ...
    def open(self, lang_to_guid: dict[str, str], movie: Movie):
        ...
        for i, (index, lang) in enumerate(ordered_tracks):
            command1 = f'--audio-track={index} {"--no-video" if i != (len(ordered_tracks) - 1) else "--video-on-top"} --aout=waveout --waveout-audio-device="{lang_to_guid[lang][:31]} ($1,$64)"'
            print("[VLC]", command1)
