...
class VLC_Instance:
    def __init__(self) -> None:
        ...
        self.id_to_player: dict[str, vlc.MediaPlayer] = {}
    ...
    def open(self, lang_to_guid: dict[str, str], movie: Movie):
        ...
        for i, (index, lang) in enumerate(ordered_tracks):
            command1 = f'--audio-track={index} {"" if i != (len(ordered_tracks) - 1) else "--video-on-top"}'
            print("[VLC]", command1)
            ...
            self.id_to_player[lang_to_guid[lang]] = player
        ...
        for player in self.players:
            player.set_time(10)
            print(f"[VLC] An interface is using {player.audio_output_device_get()}, {player.get_time()}")
        # self.set_volume(30)
    ...
    def set_volume(self, volume, guid):
        self.id_to_player[guid].audio_set_volume(volume)
        print(f"[VLC] Interface {guid} volume set to {volume}.")
    ...
    def sync(self, to_0=False):
        time = 0 if to_0 else self.players[0].get_time()
        for player in self.players:
            player.set_time(time)

    def move(self, ms):
        time = self.players[0].get_time()
        for player in self.players:
            player.set_time(time + ms)
