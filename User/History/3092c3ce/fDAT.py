...
def controls(status: dict[str, str]):
    ...
    # dynamically crate sliders for every language
    print(lang_to_guid)
    for lang, guid in lang_to_guid.items():
        # create a slider for every language
        def change_volume():
            vlc_interface.inst.set_volume(st.session_state[guid], guid)
            print(f"[WEBAPP] Changed volume of {lang} to {st.session_state[guid]}")
        ...
        st.slider(
            ...
            30,
            key=guid,
            on_change=lambda: vlc_interface.inst.set_volume(st.session_state[guid], guid),
        )
...
def select_audio(status: dict[str, str]):
    ...
    with col1:
        if st.button("Play"):
            # check if all tracks have a different interface, excluding "No interface"
            if len(set([st.session_state[track] for track in tracks if st.session_state[track] != "No interface"])) != len(
                [st.session_state[track] for track in tracks if st.session_state[track] != "No interface"]
            ):
                st.write("Please select a different interface for each language")
            else:
                track_to_guid = {
