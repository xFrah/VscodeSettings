...
def minimize_cmd_windows():
    for window in gw.getAllWindows():
        if "cmd.exe" in window.title or "Command Prompt" in window.title:
            window.minimize()


def close_edge_windows():
    for proc in psutil.process_iter():
        if "msedge.exe" in proc.name():
            proc.terminate()
...
def controls(status: dict[str, str]):
    ...
    def update_all_volumes():
        for guid in lang_to_guid.values():
