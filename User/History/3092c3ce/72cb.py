...
def controls(status: dict[str, str]):
    ...
    # dynamically create sliders with guid for key, and onchange function that changes the volume
    functions = []
    for i, (lang, guid) in enumerate(lang_to_guid.items()):

        def change_volume():
            vlc_interface.inst.set_volume(st.session_state[guid], guid)
            print(f"[WEBAPP] [{i}] Changed volume of {guid} to {st.session_state[guid]}")

        functions += [change_volume]
