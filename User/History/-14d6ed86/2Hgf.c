...
void nfc_task(void *pvParameter) {
    ...
    while (1) {
        ...
        success = pn532_readPassiveTargetID(&obj, PN532_MIFARE_ISO14443A, uid, &uidLength, 0);

        if (success) {
            // ESP_ERROR_CHECK(esp_event_post(NFCz_EVENT, NFC_EVENT_TAG_FOUND, NULL, 0, portMAX_DELAY));
...
bool nfc_init() {
    pn532_spi_init(PN532_SCK, PN532_MISO, PN532_MOSI, PN532_SS);
    pn532_begin();
    ...
    xTaskCreate(nfc_task, "nfc_task", 2048, NULL, 5, NULL);

    return true;
}

void pn532_spi_init(uint8_t clk, uint8_t miso, uint8_t mosi, uint8_t ss) {
    obj._clk = clk;
    obj._miso = miso;
    obj._mosi = mosi;
    obj._ss = ss;
