...
def ensure_receipts_folder():
    ...
    """
    folder_path = os.path.join(os.path.dirname(__file__), "receipts")
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)


def save_receipt(receipt: dict, transaction_id: str) -> None:
    """
    Saves the receipt to the database.
    """
    ensure_receipts_folder()
    folder_path = os.path.join(os.path.dirname(__file__), "receiptns")
    file_path = os.path.join(folder_path, f"{transaction_id}.json")
    with open(file_path, "w") as f:
        json.dump(receipt, f)


def get_receipt(transaction_id: str) -> Union[dict, None]:
    """
