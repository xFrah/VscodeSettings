...
def pos_server1(transaction_id, success):
    return {
...
def ack(msg, transaction_id, status):
    return {
        "packet_type": "ack",
        "seq_num": msg["seq_num"],
        "request_type": msg["packet_type"],
        "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "status": {"transaction_id": transaction_id, "state": status},
    }


def stampante_server(transaction_id, success):
    return {
        "packet_type": "stampante-server",
        "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "status": {"transaction_id": transaction_id, "state": "IDLE"},
        "success": success,
        "data": {"printedAt": "2023-10-31 16:01:00"},
    }
