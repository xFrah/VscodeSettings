...
def receive() -> None:
    ...
    if packet_type == "server-pos":
        ...
        printing_receipt(msg["transaction_id"])
...
def transaction(msg: dict):
    ...
    try:
        ...
        while trials < retries and not success:
            ...
            while time.time() - start < 8:
                ...
                if b"FINE" in buf:
                    print("FINE DETECTED---------")
                if b"TRANSAZIONE" in buf:
                    print("TRANSAZIONE DETECTED---------")
                if b"POS" in buf:
                    print("POS DETECTED---------")
                if b"NOT" in buf:
                    print("NOT DETECTED---------")
                if b"ACTIVATED" in buf:
                    print("ACTIVATED DETECTED---------")
                if (
                    b"FINE" in buf
                    and b"TRANSAZIONE" in buf
                    and b"POS" in buf
                    and b"NOT" in buf
                    and b"ACTIVATED" in buf
                ):
                    last_transaction = time.time()
                    # todo send mqtt ko
                    activate = StartOperation(
                        config["terminal_id"], cmd=StartOperation.ActivateEFTPOS
                    )
                    for _ in range(3):
    ...
    start = time.time()
    success = False
    pos_timeout = config["payment_timeout"]
    try:
...
def printing_receipt(transaction_id: str) -> None:
    """
    ...
    if receipt is not None:
        ...
        header, row1, row2, courtesy, footer = parse_receipt_string(
        ...
        success = printer.can_print()
        if success:
            message = "Receipt should be printed successfully"
        else:
            printer_status = printer.dump_printer_status_dict()
            message = f"Receipt could not be printed: {printer_status}"
    mqtt_send(packets.ricevuta_server(transaction_id, receipt, success, message))
