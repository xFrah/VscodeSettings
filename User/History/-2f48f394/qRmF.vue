    ...
    websocketData(newData) {
      if (newData) {
        ...
        newData.markers.forEach((wp) => {
          ...
          if (this.resultSeries[1].data[0] != avgHumidity) {
            let existingData = [...this.resultSeries[1].data];
            console.log('existingData1: ', existingData);
            // Update the humidity value (first value in the series)
            existingData[0] = parseInt(avgHumidity);

            console.log('existingData2: ', existingData);

            // Reassign the updated data array to the series
            this.resultSeries[1].data = existingData;

            // Trigger the update for reactivity (if needed)
            this.resultSeries = [...this.resultSeries];
            console.log('this.resultSeries: ', this.resultSeries);
          }
