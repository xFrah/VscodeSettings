    ...
    websocketData(newData) {
      if (newData) {
        ...
        if (newData.alt !== undefined) {
          ...
          this.altitudeSeries[0].data.push(parseFloat(newData.alt.toFixed(1)));
        ...
        newData.markers.forEach((wp) => {
          if (wp.id < newData.current_waypoint) {
            ...
            if (wp.temperature !== undefined) {
              resTemp.push(wp.temperature)
            }
          }

          const avgHumidity = resHumidity.reduce((a, b) => a + b, 0) / resHumidity.length;
          const avgTemperature = resTemp.reduce((a, b) => a + b, 0) / resTemp.length;
          if (this.resultSeries[1].data[0] != avgHumidity || this.resultSeries[1].data[3] != avgTemperature) {
            let existingData = [...this.resultSeries[1].data];
            existingData = [parseInt(avgHumidity), 10, 10, parseInt(avgTemperature), 10];
            this.resultSeries[1].data = existingData;
            this.resultSeries = [...this.resultSeries];
          }

          // const avgTemp = resTemp.reduce((a, b) => a + b, 0) / resTemp.length;
          // this.resultSeries[1].data.push(parseInt(avgTemp));
