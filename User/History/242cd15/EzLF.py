        ...
        with closing(connect_db()) as conn:
            ...
            for iid, date in disappearing.items():
                ...
                if date < SESSION_START_TIMESTAMP:
                    cursor.execute("DELETE FROM temp_auto_removed WHERE item_id = ?", (iid,))
                    ...
                    if (datetime.now() - date).total_seconds() > 30:
                        if occurrences[product.product_type] > 1:
                            ...
                            removed.add(iid)
                            cursor.execute("DELETE FROM temp_auto_removed WHERE item_id = ?", (iid,))
                            cursor.execute("DELETE FROM Product WHERE product_id = ?", (iid,))
            ...
            for product in products:
                ...
                if product.product_id not in rfid_rssi:
                    cursor.execute("INSERT OR IGNORE INTO temp_auto_removed (mac, item_id) VALUES (?, ?)", (self.mac, product.product_id))
...
def get_disappearing_items(kit_mac_address: str) -> Dict[str, datetime]:
    ...
    cursor.execute("SELECT item_id, last_seen FROM temp_auto_removed WHERE mac = ?", (kit_mac_address,))
...
def clearkit(mac: str):
    ...
    cursor.execute("DELETE FROM products WHERE mac = ?", (mac,))
...
def update_hash_password(username: str, password: str):
    ...
    cursor.execute("UPDATE user SET password = ? WHERE username = ?", (hasher.hash(password), username))
...
def change_kit_alias(mac: str, alias: str):
    ...
    cursor.execute("UPDATE medical_kit SET alias = ? WHERE mac = ?", (alias, mac))
...
def update_users_sha256(company_id: int):
    ...
    cursor.execute("UPDATE companies SET users_sha256 = ? WHERE index_id = ?", (sha256, company_id))
...
def get_badges_for_company(company_id: int) -> List[str]:
    ...
    cursor.execute("SELECT badge_id FROM user WHERE company_id = ? AND badge_id IS NOT NULL", (company_id,))
...
def get_kit(mac: str, company_id: int) -> List[str]:
    ...
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM medical_kit WHERE mac = ? AND company_id = ?", (mac, company_id))
    kit = cursor.fetchone()
    close_db(conn)
    if not kit:
        raise NoKitException(mac)
...
def add_user(username: str, password: str, badge_id: Union[str, None], clearance: str, company_id: int):
    ...
    cursor.execute(
        "INSERT INTO user (username, password, badge_id, clearance, company_id) VALUES (?, ?, ?, ?, ?)",
...
def get_user(username: str) -> Tuple[str, str, str, str]:
    ...
    cursor.execute("SELECT username, password, clearance, company_id FROM user WHERE username = ?", (username,))
...
def get_kits_for_company(company_id: int) -> List[Tuple[str, str]]:
    ...
    cursor.execute("SELECT mac, alias FROM medical_kit WHERE company_id = ?", (company_id,))
...
def remove_company(company_id: int):
    ...
    cursor.execute("DELETE FROM companies WHERE index_id = ?", (company_id,))
...
def register_item(item_id: str, expiry_date: str, kit_mac_address: str, item_type: str):
    ...
    cursor.execute(
        "INSERT INTO Product (product_id, product_type, expiration_date, mac) VALUES (?, ?, ?, ?)",
...
def remove_medical_kit(mac: str, company_id: int):
    ...
    cursor.execute("DELETE FROM medical_kit WHERE mac = ? AND company_id = ?", (mac, company_id))
...
def get_medical_kit_content(mac: str) -> List[Product]:
    ...
    cursor.execute("SELECT product_id, mac, product_type, expiration_date FROM Product WHERE mac = ?", (mac,))
