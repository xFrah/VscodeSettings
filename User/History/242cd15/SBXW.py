...
class KitReport:
    ...
    def decode(self, json_data: Dict[str, Any]):
        ...
        def get_removal_reason(iid, date) -> str:
            if date < SESSION_START_TIMESTAMP:
                return "it was older than session start time."
            elif iid in all_item_ids and iid in rfid_rssi:
                return "it reappeared."
            elif iid not in all_item_ids:
                return "it was not in the kit."
            else:
                return "yes."
        ...
        with closing(connect_db()) as conn:
            ...
            for iid, date in get_disappearing_items(self.mac, cursor_=cursor).items():
                ...
                if date < SESSION_START_TIMESTAMP or (iid in all_item_ids and iid in rfid_rssi) or iid not in all_item_ids:
                    cursor.execute("DELETE FROM temp_auto_removed WHERE item_id = %s", (iid,))
                    print(f"[INFO] Item {iid} removed from temp_auto_removed because {get_removal_reason(iid, date)}.")
                else:
