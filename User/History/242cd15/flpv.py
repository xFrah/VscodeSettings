...
class KitReport:
    ...
    def decode(self, json_data: Dict[str, Any]):
        ...
        with closing(connect_db()) as conn:
            ...
            for iid, date in get_disappearing_items(self.mac, cursor_=cursor).items():
                # we don't wanna risk killing items because the server was down for a while
                if date < SESSION_START_TIMESTAMP:
                    remove_from_temp_auto_removed(iid, "it was older than session start time.", cursor_=cursor)
                elif iid in all_item_ids and iid in rfid_rssi:
                    remove_from_temp_auto_removed(iid, "it reappeared.", cursor_=cursor)
                elif iid not in all_item_ids:
                    remove_from_temp_auto_removed(iid, "it was not in the kit.", cursor_=cursor)
                else:
                    ...
                    if (datetime.now() - date).total_seconds() > 30:
                        if occurrences[product.product_type] > 1:
                            occurrences[product.product_type] -= 1
                            removed.add(iid)
                            remove_from_temp_auto_removed(iid, "it was replaced", cursor_=cursor)  # TODO chain remove
            ...
            for product in products:
                ...
                if product.product_id not in rfid_rssi:
                    cursor.execute(
                        "INSERT INTO temp_auto_removed (mac, item_id) VALUES (%s, %s) ON CONFLICT (item_id) DO NOTHING",
                        (self.mac, product.product_id),
                    )
            conn.commit()
...
def get_unregistered_items(kit_mac_address: str, timeout=15.0) -> List[Tuple[str, int]]:
    ...
    raise RegistrationTimeoutException(f"No unregistered items found for {kit_mac_address}.")


def get_disappearing_items(kit_mac_address: str, cursor_=None) -> Dict[str, datetime]:
    ...
    if disappearing_items:
        ...
        return {item_id: datetime.strptime(timestamp.split(".")[0], "%Y-%m-%d %H:%M:%S") for item_id, timestamp in disappearing_items}

    return {}


def remove_from_temp_auto_removed(item_id: str, reason: str, cursor_=None):
    if cursor_ is None:
        conn = connect_db()
        cursor = conn.cursor()
    else:
        cursor = cursor_
    cursor.execute("DELETE FROM temp_auto_removed WHERE item_id = %s", (item_id,))
    if cursor_ is None:
        close_db(conn)
    print(f"[INFO] Item {item_id} removed from temp_auto_removed because {reason}.")
