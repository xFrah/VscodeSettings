...
def check_for_updates():
    ...
    if local_time[3] in hours and utime.mktime(utime.localtime()) - local_time_int > 3600:
        ...
        next_hours = min([i for i in hours if i > local_time[3]])
        # get current hour with fraction
        current_hour = utime.mktime(utime.localtime()) / 3600
        
        print(f"[UPDATER] Next check for updates will be in {local_time[3] - }.")
