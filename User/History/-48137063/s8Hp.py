...
def is_in_center(cnt, frame, threshold):
    ...
    return (
        frame.shape[1] / 2 - threshold < x < frame.shape[1] / 2 + threshold
        and frame.shape[0] / 2 - threshold < y < frame.shape[0] / 2 + threshold
    )
...
def get_diff(frame, background):
    ...
    try:
        if len(conts) == 0:
            return None, thresh, None
        biggest = max(conts, key=cv.contourArea)
        biggest = biggest if cv.contourArea(biggest) > 20 else None
        x, y, w, h = cv.boundingRect(np.concatenate(np.array([cont for cont in conts if cv.contourArea(cont) > 20])))
        x1, y1, w1, h1 = cv.boundingRect(biggest)
        return (x, y, w, h), thresh, (x1, y1, w1, h1)
    except ValueError as e:
        print(e)
        return None, thresh, None
