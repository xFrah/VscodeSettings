...
class _CombinedItemsPageState extends State<CombinedItemsPage> {
  ...
  void _initializeCombinedList(Map<String, dynamic> jsonData) {
    ...
    combinedList = missing
        .map((item) => {'value': item, 'isMissing': true})
        .toList()
      ..addAll(nullItems.map((item) => {'value': item, 'isMissing': false}));
  }
    ...
    return Scaffold(
      ...
      body: (_isRequestInProgress)
          ...
          : ListView.builder(
            ...
            return Padding(  // Outer padding
              padding: const EdgeInsets.all(8.0),
              child: Container(
                decoration: BoxDecoration(
                  border: Border.all(color: Colors.red[200]!), // Light red border
                ...
                child: Padding( // Inner padding
                  padding: const EdgeInsets.all(8.0),
                  child: ListTile(
                    contentPadding: EdgeInsets.zero, // Override the default content padding
                    minLeadingWidth: 0, // Reduce the default leading width
                    minVerticalPadding: 10, // Minimum height padding
                    title: Text(
                      item['value'],
                      style: TextStyle(
                        color: item['isMissing'] ? Colors.red : Colors.blue,
                      ),
                    ),
                    subtitle: Text(item['isMissing'] ? 'Missing' : 'Null Item'),
                  ),
                ),
              ),
            );
