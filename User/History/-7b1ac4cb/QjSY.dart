...
class _CombinedItemsPageState extends State<CombinedItemsPage> {
  ...
  void initState() {
    ...
    setState(() {
      _initializeCombinedList(widget.jsonData);
    });
  }
  ...
  void _initializeCombinedList(Map<String, dynamic> jsonData) {
    List<String>? missing_names;
    List<String> missing = List<String>.from(jsonData["missing"]);
    List<String> nullItems = List<String>.from(jsonData["null_items"]);

    // missing contains the ids of the missing items, jsonData["missing_dict"] is a map that maps them to their type
    // convert the list from ids to list of names
    missing_names = missing.map((id) => jsonData["missing_dict"][id]).cast<String>().toList();

    combinedList = missing
        .map((item) => {'value': formatString(item), 'isMissing': true})
        .toList()
      ..addAll(nullItems
          .map((item) => {'value': formatString(item), 'isMissing': false}));
  }
  ...
  Future<void> _fetchData() async {
    ...
    setState(() {
      jsonData = data;
      if (jsonData != null) {
        _initializeCombinedList(jsonData!);
      }
    });
  }
