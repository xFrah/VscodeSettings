...
class _CombinedItemsPageState extends State<CombinedItemsPage> {
  ...
  Future<Map<String, dynamic>?> get_valid_json(String mac) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse.containsKey('kit_mac_address') &&
          jsonResponse.containsKey('contents') &&
          jsonResponse.containsKey('missing_types') &&
          jsonResponse.containsKey('is_offline')) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: (_isRequestInProgress)
          ...
          : ListView.builder(
              itemCount: widget.jsonData["missing_types"].entries.length,
              itemBuilder: (context, index) {
                final MapEntry<String, dynamic> item =
                    widget.jsonData["missing_types"].entries.elementAt(index);
                return Padding(
                  // Outer padding
                  ...
                  child: Container(
                    ...
                    child: Padding(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text("Mancante x${item.value}"),
                      ),
                    ),
                  ),
                );
