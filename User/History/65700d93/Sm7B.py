...
# function that takes as argument a list of vectors, and returns a dictionary with vector as key and a list of the other vectors ordered by distance to the key vector as value
def get_nearest_neighbors(vectors):
    nearest_neighbors = {}
    for vector in vectors:
        distances = [(euclidean_distance(vector, other_vector), other_vector) for other_vector in vectors]
        distances.sort(key=lambda x: x[0])
        nearest_neighbors[vector] = [other_vector for _, other_vector in distances]
...
def test_kNN():
    ...
    ordered = get_nearest_neighbors(means)
    zipped = zip(means, class_images)

    correct = 0
    for test_image, label in zip(images_test, labels_test):
        prediction = knn(test_image, images_train, labels_train, 5, zipped, ordered)
        if prediction == label:
            correct += 1
