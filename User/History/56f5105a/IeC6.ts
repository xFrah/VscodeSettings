...
import { getOvertimeHoursCount } from "./calculation-helpers.resolver";
import { correctHeadAndTail, calculateHourlyOvertimePercentage, countOvertimeOccurrences } from "./calculation-helpers.resolver";
...
export async function getProjectDataByDate(parent, args, contextValue) {
    ...
    if (!FROM_DATE) {
        const projects = await prisma.activityWorkers.findMany({
            where: {
                ...
                project_activity_id: { select: { project_id: { select: { client: { select: { name: true } } } } } }
            }
        });
        project = projects;
    } else {
    ...
    project?.forEach((projectWorker) => {
        ...
        const clientName = projectWorker?.project_activity_id?.project_id?.client?.name
        ...
        if (projectWorker?.tasks?.length > 0) {
            projectWorker?.tasks?.forEach((taskDetails) => {
                if (taskDetails.deleted_at === null) {
                    ...
                    const overtimeMap = calculateHourlyOvertimePercentage(taskDetails?.start_time, taskDetails?.end_time, projectWorker?.project_worker_id?.worker_type);
                    let tasksOvertime = countOvertimeOccurrences(overtimeMap);
                    tasksOvertime = correctHeadAndTail(taskDetails?.start_time, taskDetails?.end_time, overtimeMap, tasksOvertime);
                    const taskDurationHours = tasksOvertime[0];
                    ordinary_hours += taskDurationHours;
                    overtime_hours += getOvertimeHoursCount(tasksOvertime)
                    let TOTAL_COST = 0;
                    ...
                    if (clientName !== "Ferie / Permesso - FraminiaECS") {
                        ...
                        console.log("project.resolver", "Ferie / Permesso - FraminiaECS");
                    }
                    console.log("project.resolver2", TOTAL_COST);
                    totalTime += minutes;
                    totalCost += TOTAL_COST;
                }
            });

            console.log("project.resolver", totalCost);
    ...
    for (const key in projectWorkers) {
        projectWorkers[key]["ordinary_hours"] = parseFloat(projectWorkers[key]["ordinary_hours"]).toFixed(2)
        projectWorkers[key]["overtime_hours"] = parseFloat(projectWorkers[key]["overtime_hours"]).toFixed(2)
