...
def get_slice_at_index(index, batch_size):
    ...
    # get the next 3 prices (normalized)
    points = [(0.5 + (p - start) / (2.0 * max_deviation))
              for p in df.iloc[label_index: label_index + 3]["Close"]]
    ...
    df_slice.loc[:, "Close"] = prices

    # now get version of dataframe with columns Close and Volume
    df_slice = df_slice[["Close", "Volume"]]
    ...
    label = torch.tensor(slope, dtype=torch.float32)
...
def plot_training_progress(train_losses, val_losses, epoch, epochs):
    ...
    plt.title(
        f"Epoch: {epoch}/{epochs}, Loss: {data_mean}, Val Loss: {val_mean}")
    ...
    for epoch in range(epochs):
        ...
        with tqdm(range(num_inputs), desc=f"Epoch {epoch+1}/{epochs}", unit="input", position=0, leave=True) as pbar:
            ...
            for s_idx, _ in zip(shuffled, pbar):
                label, df_slice, pred_list, gt_m, q = get_slice_at_index(
                    s_idx, input_size)
