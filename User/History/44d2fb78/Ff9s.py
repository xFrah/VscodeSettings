...
columns_used = ["Close", "Volume", "Open",
                "High", "Low", "Standard Deviation (20)"]
...
def get_slice_at_index(index, batch_size):
    ...
    normalized = torch.tensor(
        normalized.values, dtype=torch.float32)
    ...
    for epoch in range(epochs):
        ...
        with tqdm(range(num_inputs), desc=f"Epoch {epoch+1}/{epochs}", unit="input", position=0, leave=True) as pbar:
            ...
            for s_idx, _ in zip(shuffled, pbar):
                ...
                # Forward pass
                output, confidence = model(df_slice)
                output = output.squeeze()
                confidence = confidence.squeeze()

                # Compute loss
                loss = normal_nll(label, output, confidence)
