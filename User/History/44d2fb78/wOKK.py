...
def get_slice_at_index(index, batch_size):
    ...
    # divide unnormalized_slope in 4 different areas, so we can use it as a classification problem: [strong down, down, equal, up, strong up]
    # unnormalized_slope = -2 -> strong down
    # unnormalized_slope = -1 -> down
    # unnormalized_slope = 0 -> equal
    # unnormalized_slope = 1 -> up
    # unnormalized_slope = 2 -> strong up

    if unnormalized_slope > 0.045:
        label = [0, 0, 0, 0, 1]
    elif 0.045 > unnormalized_slope >= 0.02:
        label = [0, 0, 0, 1, 0]
    elif -0.005 <= unnormalized_slope <= 0.005:
        label = [0, 0, 1, 0, 0]
    elif -0.045 <= unnormalized_slope < -0.005:
        label = [0, 1, 0, 0, 0]
    elif unnormalized_slope < -0.045:
        label = [1, 0, 0, 0, 0]
...
def validate_training(df_val, input_size, train_losses, val_losses, loss_function, model, device):
    ...
    # Forward pass
    outputs = model(df_slice).squeeze()  # No need to squeeze since outputs are now a batch of distributions
        ...
        with tqdm(range(num_inputs), desc=f"Epoch {epoch+1}/{epochs}", unit="input", position=0, leave=True) as pbar:
            ...
            for s_idx, _ in zip(shuffled, pbar):
                ...
                if s_idx % 50 == 0:
                    ...
                    ax1.set_yscale("log")
                    # set title
                    # turn output tensor into list
                    argmax1 = torch.argmax(outputs).detach().cpu().numpy()
                    # outputlist = outputs.detach().cpu().numpy().tolist()
                
                    ax1.set_title(f"p = {[0 if i != argmax1 else 1 for i in range(5)]}, actual = {gt_m}")
                    ...
                    # argmax of outputs
                    m = torch.argmax(outputs).detach().cpu().numpy()
                    if m == 0:
                        m = -0.045
                    elif m == 1:
                        m = -0.02
                ...
                if s_idx % 10000000000000000 == 0 and False:
                    i += 1
