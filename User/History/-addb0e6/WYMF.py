...
class Waypoint(ABC):
    ...
    def has_finished(self, debug=False) -> Tuple[bool, bool]:
        """
        Returns a tuple of two booleans:
        - has_finished
        - has_failed
        """
        if self.master is None:  # waypoint has not been run yet
            return False, False
        if not self.ran:  # waypoint has not finished running
            return False, False
        if self.finished:
...
class LandWaypoint(Waypoint):
    ...
    def _run(self):
        ...
        land_at_waypoint_mission(self.master, self.lat, self.lon, self.alt_target)
        change_mode(self.master, "AUTO")  # start mission

    def _finished_condition(self):
        # TODO add stale timeout (check if vehicle is moving)
        if (mode := get_mode(self.master)) != "AUTO":
            raise WaypointFailedException(
                f"Vehicle is not in AUTO mode. ({mode} != AUTO)"
            )
        distance, (_, _, alt) = distance_to_waypoint(self.master, self.lat, self.lon)
        if not self.master.motors_armed() and distance >= 2:
            raise WaypointFailedException("Vehicle landed before reaching waypoint.")
        if self.debug:
            progress_debug(self.alt_target, distance, alt)
        return distance <= 1 and not self.master.motors_armed()

    def _failsafe(self):
        change_mode(self.master, "RTL")
