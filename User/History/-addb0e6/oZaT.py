...
from drone_helpers import Drone, MissionError, land_at_waypoint_mission
...
class SentinelMission:
    def __init__(self, waypoints: List[Tuple[float, float]], alt) -> None:
...
class LandWaypoint(Waypoint):
    ...
    def _finished_condition(self):
        ...
        if (mode := self.drone.mode) != "AUTO":
            raise WaypointFailedException(f"Vehicle is not in AUTO mode. ({mode} != AUTO)")
        distance = self.drone.distance_to_waypoint(self.lat, self.lon)
