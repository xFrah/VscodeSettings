...
def predict(model, image1_input, image2_input, device="cpu", input_size=256):
    ...
    image2 = image2.resize((image2.size[0] * 3, image2.size[1]), PIL.Image.BILINEAR)
    image1 = image1.resize((image1.size[0] * 3, image1.size[1]), PIL.Image.BILINEAR)
...
if __name__ == "__main__":
    ...
    # resize both to have twice the width
    img2 = cv2.resize(img2, (img1.shape[1] * 3, img1.shape[0]))
    img1 = cv2.resize(img1, (img1.shape[1] * 3, img1.shape[0]))
