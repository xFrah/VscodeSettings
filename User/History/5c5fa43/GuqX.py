...
def onnx_predict(onnx_model, image1_input, image2_input, input_size=256):
    ...
    print("Model input shape:", onnx_model.get_inputs()[0].shape)
    ort_inputs = {onnx_model.get_inputs()[0].name: image.cpu().numpy()}
    print("Input shape:", ort_inputs[onnx_model.get_inputs()[0].name].shape)
...
if __name__ == "__main__":
    model = onnx_load_model("models/cd_1024.onnx")
    mask = onnx_predict(model, "media/img1.png", "media/img2.png")
    mask = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
    cv2.imwrite("media/mask_onnx.png", mask)
    print("ONNX done")

    model = load_model("models/vgg16bn_iade_4_deeplabv3_PCD.pth")
    mask = predict(model, "media/img1.png", "media/img2.png")

    mask = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
    cv2.imwrite("media/mask.png", mask)
    print("PyTorch done")
