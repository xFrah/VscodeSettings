...
class AgentDDPG(AgentBase):
    ...
    def update_net(self, buffer: ReplayBuffer) -> tuple:
        ...
        for update_c in range(update_times):
            obj_critic, state = self.get_obj_critic(buffer, hyperparameters["batch_size"])
            obj_critics += obj_critic.item()
            self.optimizer_update(self.cri_optimizer, obj_critic)
            self.soft_update(self.cri_target, self.cri, hyperparameters["soft_update_tau"])
    ...
    def get_obj_critic_raw(self, buffer: ReplayBuffer, batch_size: int) -> Tuple[torch.Tensor, torch.Tensor]:
        with torch.no_grad():
    ...
    def get_obj_critic_per(self, buffer: ReplayBuffer, batch_size: int) -> Tuple[torch.Tensor, torch.Tensor]:
        with torch.no_grad():
