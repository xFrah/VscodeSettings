...
class StockTradingEnv:
    def __init__(self, initial_amount=1e6, max_stock=1e2, cost_pct=1e-3, gamma=0.99, beg_idx=0, end_idx=1113):
    ...
    def get_state(self):
        state = np.hstack(
            (
                np.tanh(np.array(self.amount * 2**-16)),
                self.shares * 2**-9,
...
class StockTradingVecEnv:
    def __init__(self, initial_amount=1e6, max_stock=1e2, cost_pct=1e-3, gamma=0.99, beg_idx=0, end_idx=1113, num_envs=4, gpu_id=0):
        self.df_pwd = "./elegantrl/envs/China_A_shares.pandas.dataframe"
        self.npz_pwd = "./elegantrl/envs/China_A_shares.numpy.npz"
        ...
        self.vmap_get_state = vmap(
            func=lambda amount, shares, close, techs: torch.cat((amount, shares, close, techs)), in_dims=(0, 0, None, None), out_dims=0
        )

        self.vmap_get_total_asset = vmap(
            func=lambda close, shares, amount: (close * shares).sum() + amount, in_dims=(None, 0, 0), out_dims=0
        )
    ...
    def step(self, action):
        ...
        for i in range(self.num_shares):
            ...
            if buy_idx.shape[0] > 0:
                ...
                self.vmap_inplace_amount_shares_when_buy(
                    part_amount, part_shares, action_int[buy_idx, i], self.close_price[self.day, i], self.cost_pct
                )
            ...
            if sell_idx.shape[0] > 0:
                ...
                self.vmap_inplace_amount_shares_when_sell(
                    part_amount, part_shares, action_int[sell_idx, i], self.close_price[self.day, i], self.cost_pct
                )
        ...
        if done:
            reward += torch.stack(self.rewards).mean(dim=0) * (1.0 / (1.0 - self.gamma))
            self.cumulative_returns = (total_asset / self.initial_amount) * 100  # todo
