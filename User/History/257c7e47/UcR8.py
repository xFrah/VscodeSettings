...
class AttentivePooling(nn.Module):
    ...
    def forward(self, inputs):
        ...
        weighted_sum = (inputs * attention_weights.unsqueeze(-1)
                        ).sum(dim=1)  # Weighted sum of input vectors
...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_characteristics_vector: Tensor, src_mask: Tensor = None) -> Tensor:
        ...
        # check if tensors have nans
        if torch.isnan(src_characteristics_vector).any():
            print("src_characteristics_vector has nans")
        ...
        # check if final output has nans
        if torch.isnan(final_output).any():
            print("final_output has nans")
