...
import { calculateHourlyOvertimePercentage, correctHeadAndTail, countOvertimeOccurrences } from "./calculation-helpers.resolver";
...
export async function workerDashboard(parent, args, contextValue) {
    ...
    tasks.forEach((task) => {
        ...
        task["overtime_hours"] = parseFloat(getOvertimeHoursCount(tasksOvertime).toFixed(2)) || 0;
        ...
        for (const overtimePercent in tasksOvertime) {
            if (overtimePercent !== "0" && tasksOvertime[overtimePercent] && CLIENT_NAME !== "Ferie / Permesso - FraminiaECS") {
     ...
     task["hours_overtime_al_15"] = parseFloat(
        ...
        let projectHours = 0;
        if (CLIENT_NAME !== "Ferie / Permesso - FraminiaECS") {
            projectHours = projectGroupByHours[projectName]
