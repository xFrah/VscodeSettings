...
class Vocabularizer:
    def __init__(self):
        ...
        if os.path.exists(r"data/datasets/forexlive_vocab.pkl"):
            self.vocab = self.load_vocab()
            print("Found file forexlive_vocab.pkl, loading vocab...")
        else:
            self.build_vocab(pd.read_csv(r"data\news\forexlive.csv")[
                             "Article"].astype(str).tolist())
    ...
    def build_vocab(self, sentences) -> None:
        self.vocab = build_vocab_from_iterator(
            map(self.tokenizer, sentences), specials=["<unk>"])
        self.vocab.set_default_index(self.vocab["<unk>"])

    def load_vocab(self):
        with open(r"data/datasets/forexlive_vocab.pkl", "rb") as f:
            vocab = pickle.load(f)
        return vocab

    def save_vocab(self) -> None:
        with open(r"data/datasets/forexlive_vocab.pkl", "wb") as f:
