...
def decode_mqtt_message(msg):
    ...
    if opmode == _STATUS_REPORT:
        ...
        try:
            ...
            for obj_name, rfid in zip(kit_columns, kit_contents):
                if rfid not in rfid_rssi:
                    print(f"RFID {obj_name} is missing.")

            unregistered = set(rfid_rssi) - set(kit_contents)
            if len(unregistered) != 0:
                print("Unregistered items found:")
                for rfid in unregistered:
                    print(f"- {rfid} ({rfid_rssi[rfid]} dBm)")
