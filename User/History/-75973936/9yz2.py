...
def decode_mqtt_message(msg):
    ...
    if opmode == _STATUS_REPORT:
        ...
        try:
            ...
            if json_data["mac"] != mac:
                print("Invalid MAC address.")
                return
            kit_contents = db.get_medical_kit_content(mac)
            if kit_contents is None:
                print("Kit not found.")
                return

            # json_data["rfid"] is a list of (rfid, rssi) tuples
            rfid_rssi = {rfid: rssi for rfid, rssi in json_data["rfid"]}

            missing: List[Tuple[str, str]] = []
            for obj_name, rfid_id in zip(kit_columns, kit_contents):
                if rfid_id not in rfid_rssi:
                    missing.append((obj_name, rfid_id))

            unregistered = set(rfid_rssi) - set(kit_contents)
            if len(unregistered) != 0:
                print("Unregistered items found:")
                for rfid_id in unregistered:
                    print(f"- {rfid_id} ({rfid_rssi[rfid_id]} dBm)")

                pushable = {rfid: rfid_rssi[rfid] for rfid in unregistered if rfid_rssi[rfid] > -60}
                db.push_unregistered_item(pushable, mac)
...
def main():
    ...
    while True:
        ...
        db.clear_old_unregistered_items()  # TODO maybe this is being done too often
        time.sleep(0.1)
