...
class _SensorPageState extends State<SensorPage> {
  ...
  Future<void> _registerItem() async {
    ...
    var response = await http.post(url, headers: {
      ...
      'expiration_date': formattedDate, // replace with actual value or variable
    ...
    // if content is not found, if the status is 200, then pContent = Item succesfully registered.
    // if the status is 401, then pContent = Login required

    if (response.statusCode == 200) {
      pContent = 'Item succesfully registered.';
    } else if (response.statusCode == 401) {
      pContent = 'Login required.';
    }
