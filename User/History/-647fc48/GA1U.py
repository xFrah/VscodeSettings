...
def change_file(idd: int, tag: bool, beam: bool):
    ...
    for i in range(len(data)):
        if "uint16_t static const BEAM_DEVICE_ID" in data[i]:
            print(f"Changing ID from {data[i]} to {idd}")
        ...
        if "#define BEAMDIGITAL_BOARD 1" in data[i]:
            ...
            print(f"Changing board from {data[i]} to {'' if beam else '//'}#define BEAMDIGITAL_BOARD 1")
            data[i] = newline
            success2 = True
        if "int k =" in data[i]:
            print(f"Changing k from {data[i]} to {'0' if tag else '2'}")
